...
pub fn unreachable_covers(test: &KernelTest, touched: &BTreeSet<String>, graph: &crate::graph::Graph, staged: &BTreeSet<String>) -> Vec<String> {
	...
	// The test kernel starts no production services before running tests. Only a fixture that
	// starts SystemManager can reach the manifest boot chain; staged bytes alone run nothing.
	if reachable.contains("bin.system_manager") {
