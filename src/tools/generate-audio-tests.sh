...
ffmpeg "${ffmpeg_args[@]}" -i "$volume/test.flac" -f s16le -c:a pcm_s16le "$root/user/flac/tests/data/test-s16le.pcm"
ffmpeg "${ffmpeg_args[@]}" -i "$volume/test.mp3" -f s16le -c:a pcm_s16le "$root/user/mp3/tests/test.pcm"
ffmpeg "${ffmpeg_args[@]}" -i "$volume/test.ogg" -f s16le -c:a pcm_s16le "$root/user/vorbis/tests/test.pcm"
ffmpeg "${ffmpeg_args[@]}" -i "$volume/test.wav" -t 0.1 -map_metadata -1 -af 'pan=stereo|c0=c0|c1=-1*c0' -c:a wavpack "$root/user/wavpack/tests/test-stereo-short.wv"
