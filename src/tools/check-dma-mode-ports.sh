...
for port in aarch64 riscv64; do
	...
	UEFI=1 SERIAL="file:$log" src/tools/guest-verdict.py dma-port-admits "$log" -- ./run.sh --arch "$port" --smp 2
	...
	UEFI=0 SERIAL="file:$log" src/tools/guest-verdict.py dma-port-admits "$log" -- src/harness/qemu-run.sh "$port" "$kernel"
	...
	UEFI=1 SERIAL="file:$log" src/tools/guest-verdict.py dma-port-degraded "$log" -- ./run.sh --arch "$port" --smp 2 --no-iommu
	...
	IOMMU=0 UEFI=0 SERIAL="file:$log" src/tools/guest-verdict.py dma-port-degraded "$log" -- src/harness/qemu-run.sh "$port" "$kernel"
	...
	echo "dma-mode-ports:   refused before a kernel was loaded"
	...
	DMA_DTB_NODE=1 UEFI=1 SERIAL="file:$log" src/tools/guest-verdict.py dma-port-loader-refused "$log" -- ./run.sh --arch "$port" --smp 1
	grep -aq "the device tree's boot-policy node is present beside this entry path's DMA-mode input" "$log" || fail "$port UEFI: the independent tree node was not named"
	echo "dma-mode-ports:   refused although the values agree"
done
