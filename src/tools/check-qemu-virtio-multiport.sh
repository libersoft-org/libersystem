...
# 1. THE DRIVER FOUND BOTH PORTS. `multiport 2/2` is announced-over-open on the two-port device, and
#    a driver without the feature reports `one port` - so this line alone separates the two.
grep -aq "driver.virtio-console: online (.*multiport 2/2" "$log" || {
	echo "qemu-virtio-multiport: the driver did not report two ports found and open" >&2
...
# 2. THE BYTES WENT TO THE PORT THEY WERE ADDRESSED TO, which is the whole claim: on the generic
...
# 3. AND THE CONSOLE IS STILL THE CONSOLE. The feature must not cost the port that ships today: the
...
echo "qemu-virtio-multiport: two ports found and opened through the control queue, and each port's bytes are on its own chardev"
