...
boot_medium() {
	local efi="$1" log="$2" verdict="$3" vars="$work/vars.$$.fd"
	cp "$OVMF_VARS" "$vars"
...
boot_medium "$efi" "$good_log" signed-clean
...
boot_medium "$efi" "$log" signed-manifest
...
boot_medium "$efi" "$payload_log" signed-payload
...
if [[ -f "$volume" ]]; then
	...
	src/tools/guest-verdict.py signed-selected-volume "$volume_log" -- qemu-system-x86_64 \
		-machine q35 -m 2G -display none -no-reboot \
		-drive "if=pflash,format=raw,readonly=on,file=$OVMF_CODE" \
		-drive "if=pflash,format=raw,file=$vars" \
		-drive "format=raw,file=$efi" \
...
# The private fixture must leave the existing image, UUID and stamp untouched, including mtime.
...
absent_list_case() {
	...
	# Snapshot the shared shape to check that private fixture production does not modify it.
	local at=0 file
	...
	# mkpackages writes the fixture and its fallback into this run's directory. In-place
	# rebuild-and-restore exposed a listless image to overlapping guests between the two copies.
	local fixture_out="$work/no-list"
	local fixture_kernel="$PWD/.build/cargo/kernel/x86_64-unknown-none/debug/kernel"
	if ! (cd src/tools/mkpackages && LIBER_OMIT_BOOTSTRAP_LIST=1 cargo run --quiet -- x86_64 system-volume "--with-kernel=$fixture_kernel" "--output-dir=$fixture_out") >"$work/omit.log" 2>&1; then
		echo "signed-boot: could not build a private volume without its bootstrap list" >&2
		return 1
	fi
	cp "$fixture_out/system-volume-bootable-x86_64.img" "$work/volume-no-list.img"
	# AND THE SHARED SHAPE IS COHERENT, asserted rather than assumed: the sidecar beside the image
	# must name the image that is actually there.
	if ! pairing_matches_volume "$(<"${BOOTABLE_SHAPE[1]}")" "$volume"; then
		echo "signed-boot: the bootable volume shape changed - its sidecar does not name the image beside it" >&2
		return 1
	fi
	...
	# about whether this gate preserved what it found. Three members, each either present with the
	...
	for file in "${BOOTABLE_SHAPE[@]}"; do
		...
		if [[ -f "$work/absent.$at" ]]; then
			[[ -e "$file" ]] && {
				echo "signed-boot: $file did not exist before this case and does now - the private build changed a shared member" >&2
				return 1
			}
		else
			[[ -f "$file" ]] || {
				echo "signed-boot: $file was here before this case and is gone - the shared shape changed" >&2
				return 1
			}
			cmp -s "$saved" "$file" || {
				echo "signed-boot: $file did not keep the bytes it had" >&2
				return 1
			}
			...
			if [[ "$(stat -c %Y.%y "$saved")" != "$(stat -c %Y.%y "$file")" ]]; then
				echo "signed-boot: $file acquired a new modification time - the build stamp is compared by WHEN it was written, so this is the mutation the shape rule forbids" >&2
				return 1
			fi
		fi
		((at += 1))
	...
	cp "$OVMF_VARS" "$vars"
	src/tools/guest-verdict.py signed-absent-list "$log" -- qemu-system-x86_64 \
		-machine q35 -m 2G -display none -no-reboot \
		-drive "if=pflash,format=raw,readonly=on,file=$OVMF_CODE" \
		-drive "if=pflash,format=raw,file=$vars" \
		-drive "format=raw,file=$efi" \
...
wrong_context() {
	...
	boot_medium "$efi" "$log" signed-context || return 1
...
wrong_volume() {
	...
	boot_medium "$efi" "$log" signed-volume-pairing || return 1
...
if [[ -f "$volume" ]]; then
	...
	if resign_with "$mixed" --release 9.9.9; then
		...
		src/tools/guest-verdict.py signed-mixed-release "$mixed_log" -- qemu-system-x86_64 \
			-machine q35 -m 2G -display none -no-reboot \
			-drive "if=pflash,format=raw,readonly=on,file=$OVMF_CODE" \
			-drive "if=pflash,format=raw,file=$vars" \
			-drive "format=raw,file=$efi" \
...
# Both fixture profiles share a run-private Cargo target, preserving the ordinary loader even
# across rebuilds with a different PE timestamp/debug identity. Each build and immutable copy
# still takes the shared producer lock; neither profile is published to the ordinary output.
...
src/tools/build-loader-private.sh test-trust "$work/loader-test-trust.efi" || fail "the test-trust loader did not build"
mcopy -o -i "$efi" "$work/loader-test-trust.efi" ::/EFI/BOOT/BOOTX64.EFI
downgrade_log="$work/downgrade-test-trust.log"
boot_medium "$efi" "$downgrade_log" signed-downgrade-test
...
LIBER_TRUST_KEY="$RELEASE_KEY" LIBER_TRUST_KEY_ID=42 src/tools/build-loader-private.sh external-release "$work/loader-release.efi" || fail "the release loader did not build"
mcopy -o -i "$efi" "$work/loader-release.efi" ::/EFI/BOOT/BOOTX64.EFI
release_log="$work/downgrade-release.log"
boot_medium "$efi" "$release_log" signed-downgrade-release
...
if [[ "${SIGNED_BOOT_PORTS:-1}" == "1" ]]; then
	...
	for port in aarch64 riscv64; do
		...
		UEFI=1 SERIAL="file:$clean_log" src/tools/guest-verdict.py signed-port-clean "$clean_log" -- ./run.sh --arch "$port" --smp 1
		...
		DAMAGE_SIGNED_MANIFEST=1 UEFI=1 SERIAL="file:$tampered_log" src/tools/guest-verdict.py signed-port-manifest "$tampered_log" -- ./run.sh --arch "$port" --smp 1
