...
grep -qF "$want" "$log" || fail "the boot did not say what it parked - expected '$want' in $log"
echo "smp-core-cap:     $want"

# THE SHOOTDOWN NEVER REFUSED. One line per boot when it does, so its absence is the assertion.
if grep -q "cores are online and this tracks" "$log"; then
	grep -m1 "cores are online and this tracks" "$log" >&2
...
if grep -q "could not be queued and its shootdown did not complete" "$log"; then
