/* The aarch64 profile target. See x86_64.h for why this is force-included.
   THE TRIPLE IS THE HARDFLOAT AAPCS VARIANT, which is what the Rust half is built with - it reports
   `target_feature="neon"` - and the pinned sources pass `double`. There are no ABI flags beside it:
   a second description of one ABI is how the two halves come to disagree. */
#ifndef LIBERSYSTEM_PROFILE_TARGET_H
#define LIBERSYSTEM_PROFILE_TARGET_H
#define LIBER_PROFILE_TRIPLE "aarch64-unknown-none"
#define LIBER_PROFILE_ARCH "aarch64"
/* Plain `char` is UNSIGNED on aarch64, as it is on riscv64; x86_64 is the target where it is
   signed. This is the one data-model value that differs across the three, which is why it is
   asserted in every translation unit rather than assumed anywhere. */
