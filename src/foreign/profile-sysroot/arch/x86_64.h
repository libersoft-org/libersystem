/* The x86_64 profile target. Force-included by the build, not by a source: every translation unit
   gets these assertions, so a disagreement stops the compile at the file that disagrees. */
#ifndef LIBERSYSTEM_PROFILE_TARGET_H
#define LIBERSYSTEM_PROFILE_TARGET_H
#define LIBER_PROFILE_TRIPLE "x86_64-unknown-none-elf"
#define LIBER_PROFILE_ARCH "x86_64"
/* Plain `char` is SIGNED on x86_64, and the substrate's character functions take `int` for exactly
   this reason: a target where it is unsigned would otherwise change what a negative byte means. */
#define LIBER_PROFILE_CHAR_SIGNED 1
