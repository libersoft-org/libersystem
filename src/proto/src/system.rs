...
impl Query {
	...
	pub(crate) fn to_json_into(&self, out: &mut String) {
		...
		match &self.source {
			Some(v95) => {
				crate::codec::json_escape(v95, out);
			}
			None => {
				out.push_str("null");
			}
...
impl NetInfo {
	...
	pub(crate) fn to_json_into(&self, out: &mut String) {
		...
		out.push('[');
		let mut v106 = true;
		for v105 in self.neighbors.iter() {
			if !v106 {
				out.push(',');
			}
			v106 = false;
	...
	pub(crate) fn to_text_into(&self, out: &mut String) {
		...
		out.push('[');
		let mut v110 = true;
		for v109 in self.neighbors.iter() {
			if !v110 {
				out.push_str(", ");
			}
			v110 = false;
