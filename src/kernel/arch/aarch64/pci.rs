...
//
// On QEMU's `virt` the PCIe ECAM is the high-mem window (base read from the device
// tree), which the boot map covers as Device memory (see paging::identity_map_device_gb).
// There is no firmware to assign BARs, so this backend also provides an MMIO-window
// allocator and drives the common `assign_bars_ecam` reassignment before resolving a
// device's layout.
...
// assigned out of it by a simple size-aligned bump. It sits below 1 GB, so the boot
// identity map already covers it as Device memory.
...
impl common::ConfigAccess for Access {
	...
	// No firmware programs the BARs on QEMU `virt`, so reassign them out of the low
