...
tagged_test!(directory_scoped_storage_clients_cannot_escape_their_grant, [Filesystem, Storage, VolumeLayout, VolumeScope], id = "kernel.volume_layout.directory_scoped_storage_clients_cannot_escape_their_grant", covers = ["liberfs", "storage"]);
...
// more - and the bytes it used to corrupt are the superblock now - so what remains
