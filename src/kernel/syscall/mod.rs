...
pub extern "C" fn syscall_dispatch(num: u64, a0: u64, a1: u64, a2: u64, a3: u64) -> u64 {
	let result: i64 = match num {
		...
		SYS_PROCESS_LIFECYCLE => sys_process_lifecycle(a0, a1, a2),
...
// Copy the caller's own `index`-th module lifecycle entry into its buffer, returning how many
// entries there are - so a runtime walks the table without knowing its length up front, exactly as
// `sys_memmap_get` walks the memory map.
//
// ITS OWN, AND NOBODY ELSE'S. There is no handle argument because there is no other process this
// could sensibly name: the entries are addresses in an address space, and an address from another
// one means nothing here. That also makes it free of any capability - a process reading what it
// must run in itself is not an authority over anything.
fn sys_process_lifecycle(index: u64, buf_ptr: u64, buf_len: u64) -> i64 {
	let Some(thread) = sched::current_thread() else { return ERR_NO_THREAD };
	let process = thread.process();
	let count = process.lifecycle_count();
	// PAST THE END IS REFUSED RATHER THAN ANSWERED WITH ZEROES. A reader that could not tell "no
	// such entry" from "an entry with nothing in it" would have no way to stop walking, and a table
	// with no entries at all is answered by the count alone: index 0 refuses and the count is 0.
	let Some(entry) = process.lifecycle_entry(index as usize) else { return ERR_INVALID };
	let size = core::mem::size_of::<abi::ModuleLifecycle>() as u64;
	if buf_len < size || !user_buf_ok(buf_ptr, size) {
		return ERR_INVALID;
	}
	if let Err(e) = write_user(buf_ptr, entry) {
		return e;
	}
	count as i64
}
...
// wait has woken it.
// WHAT A RECONSTRUCTION READS, holding no claim handle because the manager that held one died.
//
// Gated on the DeviceManager privilege, like `sys_device_claim`: this says what a device is doing,
// and a caller that may not take devices has no business enumerating who holds them.
//
// AND THE READ LATCHES. `device::snapshot` turns a `Releasing` whose deadline has passed into
// `Quarantined`, atomically, under the same lock every other claim decision is taken under - so a
// late teardown cannot put the frames and vectors back into circulation against a state a new
// manager has already been told is terminal.
fn sys_device_claim_snapshot(index: u64, privilege: u64, buf_ptr: u64, buf_len: u64) -> i64 {
	if let Err(error) = holds_privilege(privilege, PrivilegeKind::DeviceManager) {
		return error;
	}
	let size = core::mem::size_of::<abi::DeviceClaimSnapshot>() as u64;
	if buf_len < size || !user_buf_ok(buf_ptr, size) {
		return ERR_INVALID;
	}
	let Some(snapshot) = device::snapshot(index as usize) else {
		return ERR_INVALID;
	};
	if let Err(error) = write_user(buf_ptr, snapshot) {
		return error;
	}
	0
}
...
// Register the channel the kernel reports fixed-hardware platform events on.
//
// THE SAME PRIVILEGE AS THE DEVICE EVENTS ABOVE, and `abi`'s own note on this number says why: a
// separate kind would name a narrower authority than the holder already has. What this adds to a
// DeviceManager is an observation, not a power - it learns that a button on the chassis was
// pressed, and what it does about that is its own connection to the power service, which is gated
// separately and elsewhere.
fn sys_platform_events(handle: u64, privilege: u64) -> i64 {
	if let Err(error) = holds_privilege(privilege, PrivilegeKind::DeviceManager) {
		return error;
	}
	let channel = match current_typed::<Channel>(handle, ObjectType::Channel, Rights::SEND) {
	...
	crate::platform_event::attach(channel);
	crate::serial_println!("platform: a listener attached - fixed-hardware events are delivered from here");
	0
}
