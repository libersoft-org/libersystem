...
pub fn hand_off(bs: *mut BootServices, image_handle: Handle, system_table: *mut SystemTable, root: Option<*mut uefi::FileProtocol>, kernel: &[u8], reserved: &crate::ReservedKernel) -> ! {
	...
	// AND THE NAMED PROFILE IS WHERE IT IS WITHHELD - see `crate::withholds_device_tree`. The
	// firmware published one; this build declines to pass it on, which is the only way this harness
	// can present the kernel with a machine that has no tree.
...
fn build_boot_info(bs: *mut BootServices, dtb: u64, init_pkg: Option<&'static [u8]>, volume_pkg: Option<&'static [u8]>) -> u64 {
	...
	let framebuffer = if fb.present { Framebuffer { addr: fb.phys, width: fb.width, height: fb.height, pitch: fb.pitch, bytes_per_pixel: fb.bpp / 8, red_shift: fb.red_shift, red_size: fb.red_size, green_shift: fb.green_shift, green_size: fb.green_size, blue_shift: fb.blue_shift, blue_size: fb.blue_size, _pad: [0; 2] } } else { unsafe { core::mem::zeroed() } };
	unsafe {
			...
			(Some(bytes), Some(array)) => {
				...
				*entries = crate::make_module(bytes, crate::INIT_PKG_FILE, 0);
				...
				if let Some(volume) = live_volume {
					...
					crate::record_embedded_root(count as u32);
