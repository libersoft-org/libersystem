...
impl From<inflate::Error> for Error {
	fn from(error: inflate::Error) -> Error {
		match error {
			inflate::Error::Truncated => Error::Truncated,
			inflate::Error::Invalid => Error::Invalid,
			inflate::Error::Unsupported => Error::Unsupported,
			inflate::Error::TooLarge => Error::TooLarge,
		}
	}
}
