...
fn parse_format(bytes: &[u8]) -> Result<ParsedFormat, Error> {
	...
	let codec = match codec_tag {
		...
		IMA_ADPCM_FORMAT => {
			...
			if libadpcm::ima_samples_per_block(block_align, channels) != Some(samples_per_block) || byte_rate == 0 {
				return Err(Error::Invalid);
			}
		...
		MS_ADPCM_FORMAT => {
			...
			if coefficient_count == 0 || coefficient_count > 32 || extension_len < 4 + coefficient_bytes || bytes.len() < 22 + coefficient_bytes || libadpcm::ms_samples_per_block(block_align, channels) != Some(samples_per_block) || byte_rate == 0 {
				return Err(Error::Invalid);
			}
		...
		while output.len() < output_len {
			...
			if available == 0 {
				...
				match &self.wav.codec {
					Codec::Ima { samples_per_block } => libadpcm::decode_ima_block(block, self.wav.format.channels(), *samples_per_block, &mut self.pending).map_err(map_adpcm)?,
					Codec::Microsoft { samples_per_block, coefficients } => libadpcm::decode_ms_block(block, self.wav.format.channels(), *samples_per_block, coefficients, &mut self.pending).map_err(map_adpcm)?,
...
fn map_adpcm(error: libadpcm::Error) -> Error {
	match error {
		libadpcm::Error::Truncated => Error::Truncated,
		libadpcm::Error::Invalid => Error::Invalid,
		libadpcm::Error::Unsupported => Error::Unsupported,
		libadpcm::Error::TooLarge => Error::TooLarge,
	}
}
