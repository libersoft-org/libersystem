	...
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		fn fmt_rec(s :&HuffTree, f: &mut fmt::Formatter, depth :u32) -> fmt::Result {
			...
			if s.l.is_some() || s.r.is_some() {
				try_old!(writeln!(f, "ec: {:?}, pl: {:?}, LIS {:?} RIS {:?}",
					s.even_childs, s.payload, s.l.is_some(), s.r.is_some()));
			} else {
				try_old!(writeln!(f, "ec: {:?}, pl: {:?}", s.even_childs, s.payload));
			}
			if let Some(ref v) = s.l {
				depth_print!(f, depth);
				try_old!(write!(f, "LEFT "));
				try_old!(fmt_rec(&*v, f, depth + 1));
			}
			if let Some(ref v) = s.r {
				depth_print!(f, depth);
				try_old!(write!(f, "RIGT "));
				try_old!(fmt_rec(&*v, f, depth + 1));
			}
			return Ok(());
		}
		try_old!(fmt_rec(self, f, 1));
		return Ok(());
	}
} // */
		...
		if self.even_childs {
			...
			if !left.even_childs {
				if left.insert_rec(payload, depth - 1) {
					self.even_childs = left.even_childs && if let &mut Some(ref mut right) = &mut self.r.as_mut() { right.even_childs } else { false };
					return true;
				}
			}
...
impl VorbisHuffmanTree {
	...
	pub fn load_from_array(codebook_codeword_lengths: &[u8]) -> Result<VorbisHuffmanTree, HuffmanError> {
		...
		for (i, &codeword_length) in codebook_codeword_lengths.iter().enumerate() {
			...
			if !simple_tree.insert_rec(i as u32, codeword_length) {
				try_old!(Err(HuffmanError::Overspecified)) /* Overspecified, can't be put into tree */
			}
		}
		//println!("The tree:\n{:?}", simple_tree);
		...
		if cnt == 1 {
			...
			if encoded_len == 1 {
				...
				return Ok(VorbisHuffmanTree { desc_prog: vec![1u32 << 31, 3, 3, decoded as u32], unrolled_entries: [UnrolledLookupEntry::HasEntry(1, decoded as u32); 256] });
			} else {
				// Single entry codebooks must have 1 as their only length entry
				try_old!(Err(HuffmanError::InvalidSingleEntry))
			}
		}

		if !simple_tree.even_childs {
			try_old!(Err(HuffmanError::Underpopulated)); /* Underpopulated */
		}
		...
		fn uroll_traverse(tree: &HuffTree, unrolled_entries: &mut [UnrolledLookupEntry; 256], prefix: u32, prefix_idx: u8, desc_prog: &[u32], desc_prog_idx: u32) {
			let has_children = tree.l.is_some() || tree.r.is_some();
			...
			if has_children {
				...
				if prefix_idx == 8 {
					...
					// Recurse deeper.
					uroll_traverse(tree.l.as_ref().unwrap(), unrolled_entries, prefix + (0 << prefix_idx), prefix_idx + 1, desc_prog, desc_prog[desc_prog_idx as usize + 1]);
					uroll_traverse(tree.r.as_ref().unwrap(), unrolled_entries, prefix + (1 << prefix_idx), prefix_idx + 1, desc_prog, desc_prog[desc_prog_idx as usize + 2]);
				}
			} else {
		...
		return Ok(VorbisHuffmanTree { desc_prog, unrolled_entries });
	}
	...
	pub fn lookup_peeked_data<'l>(&'l self, bit_count: u8, peeked_data: u32) -> PeekedDataLookupResult<'l> {
		...
		return match self.unrolled_entries[peeked_data as usize] {
			...
			InconclusiveWithHint(hint) => Iter(8, VorbisHuffmanIter { desc_prog: &self.desc_prog, pos: hint }),
			_ => Iter(0, VorbisHuffmanIter { desc_prog: &self.desc_prog, pos: 0 }),
		};
	}
}
