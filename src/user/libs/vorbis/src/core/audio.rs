...
fn floor_zero_decode(rdr: &mut BitpackCursor, codebooks: &[Codebook], fl: &FloorTypeZero) -> Result<(Vec<f32>, u64), FloorSpecialCase> {
	// TODO this needs to become 128 bits wide, not just 64,
	// as floor0_amplitude_bits can be up to 127.
	let amplitude = try_old!(rdr.read_dyn_u64(fl.floor0_amplitude_bits));
	if amplitude <= 0 {
	...
	let booknumber = try_old!(rdr.read_dyn_u32(ilog(fl.floor0_number_of_books as u64)));
	match fl.floor0_book_list.get(booknumber as usize) {
		// Undecodable per spec
		None => try_old!(Err(FloorSpecialCase::PacketUndecodable)),
		Some(codebook_idx) => {
			...
			loop {
				...
				let temp_vector = try_old!(rdr.read_huffman_vq(codebook));
				if temp_vector.len() + coefficients.len() < fl.floor0_order as usize {
					...
					for &e in temp_vector {
						coefficients.push(libm::cosf(last + e as f32));
						last_new = e as f32;
					}
				} else {
					for &e in temp_vector {
...
fn floor_zero_compute_curve(cos_coefficients: &[f32], amplitude: u64, fl: &FloorTypeZero, blockflag: bool, n: u16) -> Vec<f32> {
	let cached_bark_cos_omega = &fl.cached_bark_cos_omega[blockflag as usize];
	let mut i = 0;
	let mut output = Vec::with_capacity(n as usize);
	let lfv_common_term = amplitude as f32 * fl.floor0_amplitude_offset as f32 / ((1 << fl.floor0_amplitude_bits) - 1) as f32;
	while i < n as usize {
		...
		let (mut p, mut q) = if fl.floor0_order & 1 == 1 { (1.0 - cos_omega * cos_omega, 0.25) } else { ((1.0 - cos_omega) / 2.0, (1.0 + cos_omega) / 2.0) };
		...
		let linear_floor_value = libm::expf(0.11512925 * (lfv_common_term / libm::sqrtf(p + q) - fl.floor0_amplitude_offset as f32));
...
fn floor_one_decode(rdr: &mut BitpackCursor, codebooks: &[Codebook], fl: &FloorTypeOne) -> Result<Vec<u32>, FloorSpecialCase> {
	...
	let b = ilog(range - 1);
	floor1_y.push(try_old!(rdr.read_dyn_u8(b)) as u32);
	floor1_y.push(try_old!(rdr.read_dyn_u8(b)) as u32);
	...
	for class in &fl.floor1_partition_class {
		...
		if cbits > 0 {
			...
			cval = try_old!(rdr.read_huffman(&codebooks[cbook].codebook_huffman_tree));
		}
		for _ in 0..cdim {
			...
			if book >= 0 {
				...
				floor1_y.push(try_old!(rdr.read_huffman(tree)));
			} else {
				floor1_y.push(0);
			}
		}
	...
	F: Fn(u32, u32) -> core::cmp::Ordering,
{
	...
	let min_idx = prefix.iter().position(|&val| smaller(val, bound)).unwrap_or_else(|| panic!("No index y < {0} found where v[y] is {1} than v[{0}] = 0x{2:08x}!", max_idx, relation, bound));
...
fn floor_one_curve_compute_amplitude(floor1_y: &[u32], fl: &FloorTypeOne) -> (Vec<u32>, Vec<bool>) {
	...
	for (i, el) in fl.floor1_x_list.iter().enumerate().skip(2) {
		...
		let predicted = render_point(cur_low_neighbor.1, floor1_final_y[cur_low_neighbor.0], cur_high_neighbor.1, floor1_final_y[cur_high_neighbor.0], *el) as i32;
		...
		if val > 0 {
			...
			floor1_step2_flag.push(true);
			floor1_final_y.push(if val >= room { if highroom > lowroom { predicted + val - lowroom } else { predicted - val + highroom - 1 } } else { predicted + (if val % 2 == 1 { -val - 1 } else { val } >> 1) } as u32);
		} else {
...
fn floor_one_curve_synthesis(floor1_final_y: Vec<u32>, floor1_step2_flag: Vec<bool>, fl: &FloorTypeOne, n: u16) -> Vec<f32> {
	...
	floor.into_iter().map(|idx| FLOOR1_INVERSE_DB_TABLE[idx as usize]).collect()
}

fn floor_decode<'a>(rdr: &mut BitpackCursor, ident: &IdentHeader, mapping: &Mapping, codebooks: &[Codebook], floors: &'a [Floor]) -> Result<Vec<DecodedFloor<'a>>, ()> {
	...
	for i in 0..ident.audio_channels as usize {
		...
		let floor_res = match floor {
			&Floor::TypeZero(ref fl) => match floor_zero_decode(rdr, codebooks, fl) {
				...
				Err(PacketUndecodable) => try_old!(Err(())),
			},
...
fn residue_packet_read_partition(rdr: &mut BitpackCursor, codebook: &Codebook, resid: &Residue, vec_v: &mut [f32]) -> Result<(), HuffmanVqReadErr> {
	if resid.residue_type == 0 {
		...
		for i in 0..step {
			let entry_temp = try_old!(rdr.read_huffman_vq(codebook));
		...
		while i < partition_size {
			let entries = try_old!(rdr.read_huffman_vq(codebook));
...
fn residue_packet_decode_inner(rdr: &mut BitpackCursor, cur_blocksize: u16, do_not_decode_flag: &[bool], resid: &Residue, codebooks: &[Codebook]) -> Result<Vec<f32>, ()> {
				...
				if pass == 0 {
					for (j, do_not_decode) in do_not_decode_flag.iter().enumerate() {
						...
						for i in (0..classwords_per_codeword).rev() {
							classifications[j * cl_stride + i + partition_count] = temp % resid.residue_classifications as u32;
...
fn residue_packet_decode(rdr: &mut BitpackCursor, cur_blocksize: u16, do_not_decode_flag: &[bool], resid: &Residue, codebooks: &[Codebook]) -> Result<Vec<f32>, ()> {
	let ch = do_not_decode_flag.len();
	...
	if resid.residue_type == 2 {
		...
		if !to_decode_found {
			...
			let vectors = try_old!(residue_packet_decode_inner(rdr, cur_blocksize * ch as u16, &c_do_not_decode_flag, resid, codebooks));
...
fn inverse_couple(m: f32, a: f32) -> (f32, f32) {
	if m > 0. { if a > 0. { (m, m - a) } else { (m + a, m) } } else { if a > 0. { (m, m + a) } else { (m - a, m) } }
}
...
fn dct_iv_slow(buffer: &mut [f32]) {
	...
	let n = buffer.len();
	let nmask = (n << 3) - 1;
	let mcos = (0..8 * n).map(|i| libm::cosf(core::f32::consts::FRAC_PI_4 * (i as f32) / (n as f32))).collect::<Vec<_>>();
	for i in 0..n {
		let mut acc = 0.;
		for j in 0..n {
...
*/
pub fn get_decoded_sample_count(ident: &IdentHeader, setup: &SetupHeader, packet: &[u8]) -> Result<usize, AudioReadError> {
	let mut rdr = BitpackCursor::new(packet);
	if try_old!(rdr.read_bit_flag()) {
		try_old!(Err(AudioReadError::AudioIsHeader));
	}
	let mode_number = try_old!(rdr.read_dyn_u8(ilog(setup.modes.len() as u64 - 1)));
	let mode = &setup.modes[mode_number as usize];
	let bs = if mode.mode_blockflag { ident.blocksize_1 } else { ident.blocksize_0 };
	let n: u16 = 1 << bs;
	let previous_next_window_flag = if mode.mode_blockflag { Some((try_old!(rdr.read_bit_flag()), try_old!(rdr.read_bit_flag()))) } else { None };
	// Compute windowing info for left window
	let window_center = n >> 1;
	let (left_win_start, _left_win_end, _left_n, _left_n_use_bs1) = if previous_next_window_flag.map_or(true, |(prev_win_flag, _)| prev_win_flag) {
	...
	let (right_win_start, _right_win_end) = if previous_next_window_flag.map_or(true, |(_, next_win_flag)| next_win_flag) {
		(window_center, n)
	} else {
		let bs_0_exp = 1 << ident.blocksize_0;
...
*/
pub fn read_audio_packet_generic<S: Samples>(ident: &IdentHeader, setup: &SetupHeader, packet: &[u8], pwr: &mut PreviousWindowRight) -> Result<S, AudioReadError> {
	let mut rdr = BitpackCursor::new(packet);
	if try_old!(rdr.read_bit_flag()) {
		try_old!(Err(AudioReadError::AudioIsHeader));
	}
	let mode_number = try_old!(rdr.read_dyn_u8(ilog(setup.modes.len() as u64 - 1)));
	let mode = if let Some(mode) = setup.modes.get(mode_number as usize) { mode } else { try_old!(Err(AudioReadError::AudioBadFormat)) };
	let mapping = &setup.mappings[mode.mode_mapping as usize];
	let bs = if mode.mode_blockflag { ident.blocksize_1 } else { ident.blocksize_0 };
	let n: u16 = 1 << bs;
	let previous_next_window_flag = if mode.mode_blockflag { Some((try_old!(rdr.read_bit_flag()), try_old!(rdr.read_bit_flag()))) } else { None };
	// Decode the floors
	let decoded_floor_infos = try_old!(floor_decode(&mut rdr, ident, mapping, &setup.codebooks, &setup.floors));
	...
	// Inverse coupling
	for (&mag, &angle) in mapping.mapping_magnitudes.iter().rev().zip(mapping.mapping_angles.iter().rev()) {
	...
	for (residue_vector, chan_decoded_floor) in residue_vectors.iter().zip(decoded_floor_infos.iter()) {
		let mut floor_decoded: Vec<f32> = match chan_decoded_floor {
			&DecodedFloor::TypeZero(ref coefficients, amplitude, ref fl) => floor_zero_compute_curve(coefficients, amplitude, fl, mode.mode_blockflag, n / 2),
		...
		for (prev_chan, chan) in prev_data.into_iter().zip(audio_spectri.iter_mut()) {
			...
			let (lhs, rhs) = {
				if win_slope.len() < plen {
					...
					try_old!(Err(AudioReadError::AudioBadFormat));
				}
