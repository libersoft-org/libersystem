// @generated by lsidl-gen from src/idl/display.lsidl (package liber:display@1).
...
pub mod display {
	...
	pub fn events_open<S: Service>(service: &mut S, request: &[u8], request_handles: &mut Handles) -> Option<(u32, Vec<DisplayEvent>)> {
		let mut reader = Reader::with_handle_list(request, request_handles);
		let r = &mut reader;
		let _op = r.u16()?;
		let corr = r.u32()?;
	...
	pub fn events_read(msg: &[u8], frame_handles: &mut Handles) -> Option<DisplayEvent> {
