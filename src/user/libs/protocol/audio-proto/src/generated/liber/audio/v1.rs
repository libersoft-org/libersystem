...
pub mod audio {
	...
	pub fn dispatch<S: Service>(service: &mut S, request: &[u8], request_handles: &mut Handles, out: &mut [u8], reply_handles: &mut Handles) -> Option<usize> {
		...
		match op {
			...
			OP_OPEN_CAPTURE => {
				...
				r.finish()?;
				request_handles.clear();
				let result = service.open_capture(rate, channels);
				let encoded: Option<()> = (|| {
					let w = &mut writer;
					w.u32(corr)?;
					match &result {
	...
	#[cfg(feature = "channel-client-impl")]
	#[inline(never)]
	#[unsafe(export_name = "liber_channel_impl_liber_audio_audio_open_capture")]
	fn channel_invoke_open_capture(chan: u64, rate: &u32, channels: &u8) -> Option<Result<u64, Error>> {
		let mut client = Client::new(ipc_client::ChannelTransport { chan });
...
// interface `pcm-capture` over a channel: opcodes, a Service trait + dispatch, and a Client.
pub mod pcm_capture {
	use super::*;
	use crate::codec::{Reader, Sink, SliceWriter, Transport, TransportError, VecWriter};
	use alloc::vec::Vec;
	...
	#[cfg(feature = "channel-client-impl")]
	#[inline(never)]
	#[unsafe(export_name = "liber_channel_impl_liber_audio_pcm_capture_read")]
	...
	#[cfg(feature = "channel-client-impl")]
	#[inline(never)]
	#[unsafe(export_name = "liber_channel_impl_liber_audio_pcm_capture_close")]
	fn channel_invoke_close(chan: u64) -> Option<Result<(), Error>> {
		let mut client = Client::new(ipc_client::ChannelTransport { chan });
		client.close()
	}
...
pub mod audio_admin {
	...
	pub fn dispatch<S: Service>(service: &mut S, request: &[u8], request_handles: &mut Handles, out: &mut [u8], reply_handles: &mut Handles) -> Option<usize> {
		...
		match op {
			...
			OP_OPEN_CAPTURES => {
				r.finish()?;
				request_handles.clear();
				let result = service.open_captures();
				let encoded: Option<()> = (|| {
					let w = &mut writer;
					w.u32(corr)?;
					match &result {
	...
	#[cfg(feature = "channel-client-impl")]
	#[inline(never)]
	#[unsafe(export_name = "liber_channel_impl_liber_audio_audio_admin_open_captures")]
