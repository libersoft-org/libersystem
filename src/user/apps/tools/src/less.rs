...
fn index_lines(storage: u64, uri: &str, held: &[u8], streamed: bool) -> Option<Vec<u64>> {
	unsafe {
...
fn line_bytes(storage: u64, uri: &str, held: &[u8], streamed: bool, lines: &[u64], index: usize) -> Vec<u8> {
	unsafe {
...
fn page(output: &mut impl TerminalWriter, storage: u64, uri: &str, held: &[u8], streamed: bool, lines: &[u64], view: &mut View) {
	let input = stdin();
...
fn read_pattern(output: &mut impl TerminalWriter, input: u64, decoder: &mut InputDecoder, view: &View, forward: bool) -> Option<Vec<u8>> {
...
fn search(storage: u64, uri: &str, held: &[u8], streamed: bool, lines: &[u64], view: &mut View, forward: bool) {
	view.notice.clear();
...
fn render(output: &mut impl TerminalWriter, storage: u64, uri: &str, held: &[u8], streamed: bool, lines: &[u64], view: &View) -> bool {
