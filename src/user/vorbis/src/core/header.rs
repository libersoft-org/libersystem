...
const MAX_CODEBOOK_ENTRIES: u64 = 262_144;
const MAX_CODEBOOK_VECTOR_VALUES: u64 = 1_048_576;
...
macro_rules! convert_to_usize {
	( $val:expr, $val_type:ident ) => {{
		...
		if $val != converted as $val_type {
			try_old!(Err(HeaderReadError::BufferNotAddressable));
		}
		converted
	}};
}
...
( $rdr:expr ) => { {
	...
	if res & 1 == 0 {
		...
		try_old!(Err(HeaderReadError::HeaderIsAudio));
	}
	let is_vorbis =
		try_old!($rdr.read_u8()) == 0x76 && // 'v'
		try_old!($rdr.read_u8()) == 0x6f && // 'o'
		try_old!($rdr.read_u8()) == 0x72 && // 'r'
		try_old!($rdr.read_u8()) == 0x62 && // 'b'
		try_old!($rdr.read_u8()) == 0x69 && // 'i'
		try_old!($rdr.read_u8()) == 0x73;   // 's'
	if !is_vorbis {
		try_old!(Err(HeaderReadError::NotVorbisHeader));
	}
	return Ok(res);
}}
}
...
fn read_header_begin_cursor(rdr: &mut ByteCursor<'_>) -> Result<u8, HeaderReadError> {
	read_header_begin_body!(rdr)
}
...
impl<'a> ByteCursor<'a> {
	fn new(packet: &'a [u8]) -> Self {
		Self { packet, position: 0 }
	}

	fn read_u8(&mut self) -> Result<u8, HeaderReadError> {
		let value = *self.packet.get(self.position).ok_or(HeaderReadError::EndOfPacket)?;
		self.position += 1;
		Ok(value)
	}

	fn read_u32(&mut self) -> Result<u32, HeaderReadError> {
		let bytes = self.read_bytes(4)?;
		Ok(u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]))
	}

	fn read_bytes(&mut self, length: usize) -> Result<&'a [u8], HeaderReadError> {
		let end = self.position.checked_add(length).ok_or(HeaderReadError::BufferNotAddressable)?;
		let bytes = self.packet.get(self.position..end).ok_or(HeaderReadError::EndOfPacket)?;
		self.position = end;
		Ok(bytes)
	}

	fn remaining(&self) -> usize {
		self.packet.len() - self.position
	}
}
...
pub fn read_header_ident(packet: &[u8]) -> Result<IdentHeader, HeaderReadError> {
	...
	let hd_id = try_old!(read_header_begin(&mut rdr));
	if hd_id != 1 {
		try_old!(Err(HeaderReadError::HeaderBadType(hd_id)));
	}
	let vorbis_version = try_old!(rdr.read_u32());
	if vorbis_version != 0 {
		try_old!(Err(HeaderReadError::UnsupportedVorbisVersion));
	}
...
pub fn read_header_comment(packet: &[u8]) -> Result<CommentHeader, HeaderReadError> {
	let mut rdr = ByteCursor::new(packet);
	let hd_id = try_old!(read_header_begin_cursor(&mut rdr));
	if hd_id != 3 {
		try_old!(Err(HeaderReadError::HeaderBadType(hd_id)));
	}
	// First read the vendor string
	let vendor_length = try_old!(rdr.read_u32()) as usize;
	let vendor = try_old!(String::from_utf8(try_old!(rdr.read_bytes(vendor_length)).to_vec()));

	// Now read the comments
	let comment_count = try_old!(rdr.read_u32()) as usize;
	if comment_count > MAX_COMMENTS {
		return Err(HeaderReadError::BufferNotAddressable);
	}
	if comment_count > rdr.remaining() / 4 {
		return Err(HeaderReadError::EndOfPacket);
	}
	let mut comment_list = Vec::with_capacity(comment_count);
	for _ in 0..comment_count {
		let comment_length = try_old!(rdr.read_u32()) as usize;
		let comment = match String::from_utf8(try_old!(rdr.read_bytes(comment_length)).to_vec()) {
			Ok(comment) => comment,
		...
		let eq_idx = match comment.find("=") {
			...
			None => continue, // try_old!(Err(HeaderReadError::HeaderBadFormat))
		};
...
impl ResidueBook {
	pub fn get_val(&self, i: u8) -> Option<u8> {
		...
		return if self.vals_used & (1 << i) > 0 { Some(self.val_i[i as usize]) } else { None };
	}
	...
	fn read_book(rdr: &mut BitpackCursor, vals_used: u8, codebooks: &[Codebook]) -> Result<Self, HeaderReadError> {
		...
		for i in 0..7 {
			...
			let val_entry = try_old!(rdr.read_u8());
	...
	if lup.codebook_lookup_type == 1 {
		...
		for lookup_offset in 0..codebook_entries {
			...
			for _ in 0..codebook_dimensions {
				...
				let vec_elem = lup.codebook_multiplicands[multiplicand_offset] as f32 * lup.codebook_delta_value + lup.codebook_minimum_value + last;
...
struct SetupBudget {
	...
	codebook_vector_values: u64,
}

impl SetupBudget {
	fn claim_codebook(&mut self, entries: u32) -> Result<(), HeaderReadError> {
		self.codebook_entries = self.codebook_entries.checked_add(entries as u64).ok_or(HeaderReadError::BufferNotAddressable)?;
		if self.codebook_entries > MAX_CODEBOOK_ENTRIES {
			return Err(HeaderReadError::BufferNotAddressable);
		}
		Ok(())
	}

	fn claim_vectors(&mut self, entries: u32, dimensions: u16) -> Result<(), HeaderReadError> {
		let values = (entries as u64).checked_mul(dimensions as u64).ok_or(HeaderReadError::BufferNotAddressable)?;
		self.codebook_vector_values = self.codebook_vector_values.checked_add(values).ok_or(HeaderReadError::BufferNotAddressable)?;
		if self.codebook_vector_values > MAX_CODEBOOK_VECTOR_VALUES {
			return Err(HeaderReadError::BufferNotAddressable);
		}
		Ok(())
	}
...
fn read_codebook(rdr: &mut BitpackCursor, budget: &mut SetupBudget) -> Result<Codebook, HeaderReadError> {
	// 1. Read the sync pattern
	let sync_pattern = try_old!(rdr.read_u24());
	if sync_pattern != 0x564342 {
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}

	// 2. Read the _dimension, _entries fields and the ordered bitflag
	let codebook_dimensions = try_old!(rdr.read_u16());
	let codebook_entries = try_old!(rdr.read_u24());
	if codebook_dimensions == 0 {
		return Err(HeaderReadError::HeaderBadFormat);
	}
	try_old!(budget.claim_codebook(codebook_entries));
	let ordered = try_old!(rdr.read_bit_flag());

	// 3. Read the codeword lengths
	let mut codebook_codeword_lengths = Vec::with_capacity(convert_to_usize!(codebook_entries, u32));
	if !ordered {
		let sparse = try_old!(rdr.read_bit_flag());
		for _ in 0..codebook_entries {
			let length = if sparse {
				let flag = try_old!(rdr.read_bit_flag());
				if flag {
					try_old!(rdr.read_u5()) + 1
				} else {
		...
		let mut current_length = try_old!(rdr.read_u5()) + 1;
		while current_entry < codebook_entries {
			let number = try_old!(rdr.read_dyn_u32(crate::ilog((codebook_entries - current_entry) as u64)));
	...
	let codebook_lookup_type = try_old!(rdr.read_u4());
	...
	let codebook_lookup: Option<CodebookVqLookup> = if codebook_lookup_type == 0 {
		...
		try_old!(budget.claim_vectors(codebook_entries, codebook_dimensions));
		let codebook_minimum_value = try_old!(rdr.read_f32());
		let codebook_delta_value = try_old!(rdr.read_f32());
		let codebook_value_bits = try_old!(rdr.read_u4()) + 1;
		let codebook_sequence_p = try_old!(rdr.read_bit_flag());
		let codebook_lookup_values: u64 = if codebook_lookup_type == 1 { lookup1_values(codebook_entries, codebook_dimensions) as u64 } else { codebook_entries as u64 * codebook_dimensions as u64 };
		...
		for _ in 0..codebook_lookup_values {
			codebook_multiplicands.push(try_old!(rdr.read_dyn_u32(codebook_value_bits)));
		}
		Some(CodebookVqLookup { codebook_lookup_type, codebook_minimum_value, codebook_delta_value, codebook_sequence_p, codebook_multiplicands })
	};
...
fn read_floor(rdr: &mut BitpackCursor, codebook_cnt: u16, blocksizes: (u8, u8)) -> Result<Floor, HeaderReadError> {
	let floor_type = try_old!(rdr.read_u16());
	match floor_type {
		0 => {
			let floor0_order = try_old!(rdr.read_u8());
			let floor0_rate = try_old!(rdr.read_u16());
			let floor0_bark_map_size = try_old!(rdr.read_u16());
			let floor0_amplitude_bits = try_old!(rdr.read_u6());
			...
			let floor0_amplitude_offset = try_old!(rdr.read_u8());
			let floor0_number_of_books = try_old!(rdr.read_u4()) + 1;
			let mut floor0_book_list = Vec::with_capacity(convert_to_usize!(floor0_number_of_books, u8));
			for _ in 0..floor0_number_of_books {
				let value = try_old!(rdr.read_u8());
		...
		1 => {
			let floor1_partitions = try_old!(rdr.read_u5());
			...
			for _ in 0..floor1_partitions {
				let cur_class = try_old!(rdr.read_u4());
				maximum_class = if cur_class as i8 > maximum_class { cur_class as i8 } else { maximum_class };
			...
			for _ in 0..maximum_class + 1 {
				floor1_class_dimensions.push(try_old!(rdr.read_u3()) + 1);
				let cur_subclass = try_old!(rdr.read_u2());
				floor1_class_subclasses.push(cur_subclass);
				if cur_subclass != 0 {
					let cur_masterbook = try_old!(rdr.read_u8());
				...
				for _ in 0..cur_books_cnt {
					...
					let cur_book = (try_old!(rdr.read_u8()) as i16) - 1;
			...
			let floor1_multiplier = try_old!(rdr.read_u2()) + 1;
			let rangebits = try_old!(rdr.read_u4());
			...
			for cur_class_num in &floor1_partition_class_list {
				for _ in 0..floor1_class_dimensions[*cur_class_num as usize] {
					floor1_x_list.push(try_old!(rdr.read_dyn_u32(rangebits)));
				}
			}
			...
			Ok(Floor::TypeOne(FloorTypeOne { floor1_multiplier, floor1_partition_class: floor1_partition_class_list, floor1_class_dimensions, floor1_class_subclasses, floor1_subclass_books, floor1_class_masterbooks, floor1_x_list, floor1_x_list_sorted }))
		}
...
fn read_residue(rdr: &mut BitpackCursor, codebooks: &[Codebook]) -> Result<Residue, HeaderReadError> {
	let residue_type = try_old!(rdr.read_u16());
	if residue_type > 2 {
		// Undecodable by spec
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}
	let residue_begin = try_old!(rdr.read_u24());
	let residue_end = try_old!(rdr.read_u24());
	...
	let residue_partition_size = try_old!(rdr.read_u24()) + 1;
	let residue_classifications = try_old!(rdr.read_u6()) + 1;
	let residue_classbook = try_old!(rdr.read_u8());
	...
	for _ in 0..residue_classifications {
		let mut high_bits = 0;
		let low_bits = try_old!(rdr.read_u3());
		let bitflag = try_old!(rdr.read_bit_flag());
		if bitflag {
			high_bits = try_old!(rdr.read_u5());
		}
	...
	for cascade_entry in &residue_cascade {
		residue_books.push(try_old!(ResidueBook::read_book(rdr, *cascade_entry, codebooks)));
	}
	...
	return Ok(Residue { residue_type: residue_type as u8, residue_begin, residue_end, residue_partition_size, residue_classifications, residue_classbook, residue_books });
}

/// Reads a "Mapping" which is part of the setup header packet.
fn read_mapping(rdr: &mut BitpackCursor, audio_chan_ilog: u8, audio_channels: u8, floor_count: u8, residue_count: u8) -> Result<Mapping, HeaderReadError> {
	let mapping_type = try_old!(rdr.read_u16());
	if mapping_type > 0 {
		// Undecodable per spec
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}
	let mapping_submaps = match try_old!(rdr.read_bit_flag()) {
		true => try_old!(rdr.read_u4()) + 1,
		false => 1,
	};
	let mapping_coupling_steps = match try_old!(rdr.read_bit_flag()) {
		true => try_old!(rdr.read_u8()) as u16 + 1,
		false => 0,
	};
	...
	for _ in 0..mapping_coupling_steps {
		let cur_mag = try_old!(rdr.read_dyn_u8(audio_chan_ilog));
		let cur_angle = try_old!(rdr.read_dyn_u8(audio_chan_ilog));
	...
	let reserved = try_old!(rdr.read_u2());
	if reserved != 0 {
		// Undecodable per spec
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}
	...
	for _ in 0..mapping_submaps {
		...
		try_old!(rdr.read_u8());
		let cur_floor = try_old!(rdr.read_u8());
		let cur_residue = try_old!(rdr.read_u8());
...
fn read_mode_info(rdr: &mut BitpackCursor, mapping_count: u8) -> Result<ModeInfo, HeaderReadError> {
	let mode_blockflag = try_old!(rdr.read_bit_flag());
	let mode_windowtype = try_old!(rdr.read_u16());
	let mode_transformtype = try_old!(rdr.read_u16());
	let mode_mapping = try_old!(rdr.read_u8());
	// Verifying ranges
	if mode_windowtype != 0 || mode_transformtype != 0 || mode_mapping >= mapping_count {
		// Undecodable per spec
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}
	return Ok(ModeInfo { mode_blockflag, mode_mapping });
}
...
pub fn read_header_setup(packet: &[u8], audio_channels: u8, blocksizes: (u8, u8)) -> Result<SetupHeader, HeaderReadError> {
	let mut rdr = BitpackCursor::new(packet);
	let hd_id = try_old!(read_header_begin(&mut rdr));
	if hd_id != 5 {
		try_old!(Err(HeaderReadError::HeaderBadType(hd_id)));
	...
	let vorbis_codebook_count: u16 = try_old!(rdr.read_u8()) as u16 + 1;
	let mut codebooks = Vec::with_capacity(vorbis_codebook_count as usize);
	let mut setup_budget = SetupBudget { codebook_entries: 0, codebook_vector_values: 0 };
	for _ in 0..vorbis_codebook_count {
		codebooks.push(try_old!(read_codebook(&mut rdr, &mut setup_budget)));
	}

	// 2. Read the time domain transforms
	let vorbis_time_count: u8 = try_old!(rdr.read_u6()) + 1;
	for _ in 0..vorbis_time_count {
		if try_old!(rdr.read_u16()) != 0 {
			try_old!(Err(HeaderReadError::HeaderBadFormat));
		}
	}

	// 3. Read the floor values
	let vorbis_floor_count: u8 = try_old!(rdr.read_u6()) + 1;
	let mut floors = Vec::with_capacity(vorbis_floor_count as usize);
	for _ in 0..vorbis_floor_count {
		floors.push(try_old!(read_floor(&mut rdr, vorbis_codebook_count, blocksizes)));
	}

	// 4. Read the residue values
	let vorbis_residue_count: u8 = try_old!(rdr.read_u6()) + 1;
	let mut residues = Vec::with_capacity(vorbis_residue_count as usize);
	for _ in 0..vorbis_residue_count {
		residues.push(try_old!(read_residue(&mut rdr, &codebooks)));
	}

	// 5. Read the mappings
	let vorbis_mapping_count: u8 = try_old!(rdr.read_u6()) + 1;
	let mut mappings = Vec::with_capacity(vorbis_mapping_count as usize);
	for _ in 0..vorbis_mapping_count {
		mappings.push(try_old!(read_mapping(&mut rdr, audio_chan_ilog, audio_channels, vorbis_floor_count, vorbis_residue_count)));
	}

	// 6. Read the modes
	let vorbis_mode_count: u8 = try_old!(rdr.read_u6()) + 1;
	let mut modes = Vec::with_capacity(vorbis_mode_count as usize);
	for _ in 0..vorbis_mode_count {
		modes.push(try_old!(read_mode_info(&mut rdr, vorbis_mapping_count)));
	}

	// Now we only have to make sure the framing bit is set,
	// and we can successfully return the setup header!
	let framing: bool = try_old!(rdr.read_bit_flag());
	if !framing {
		try_old!(Err(HeaderReadError::HeaderBadFormat));
	}

	return Ok(SetupHeader { codebooks, floors, residues, mappings, modes });
}
