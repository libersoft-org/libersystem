...
impl CachedBlocksizeDerived {
	pub fn from_blocksize(bs: u8) -> Self {
		CachedBlocksizeDerived { window_slope: generate_window((1 << (bs as u16)) >> 1), twiddle_factors: compute_twiddle_factors(bs), bitrev: compute_bitreverse(bs) }
	}
}
...
fn win_slope(x: u16, n: u16) -> f32 {
	...
	let v = libm::sinf(0.5 * core::f32::consts::PI * (x as f32 + 0.5) / n as f32);
	return libm::sinf(0.5 * core::f32::consts::PI * v * v);
}
...
fn compute_twiddle_factors(blocksize: u8) -> TwiddleFactors {
	...
	for k in 0..n4 {
		a.push(libm::cosf((k as f32) * pi_4_n));
		a.push(-libm::sinf((k as f32) * pi_4_n));
		b.push(libm::cosf(((k2 + 1) as f32) * pi_05_n) * 0.5);
		b.push(libm::sinf(((k2 + 1) as f32) * pi_05_n) * 0.5);
		k2 += 2;
	}
	k2 = 0;
	for _ in 0..n8 {
		c.push(libm::cosf(((k2 + 1) as f32) * pi_2_n));
		c.push(-libm::sinf(((k2 + 1) as f32) * pi_2_n));
		k2 += 2;
	}
...
pub fn compute_bark_map_cos_omega(n: u16, floor0_rate: u16, floor0_bark_map_size: u16) -> Vec<f32> {
	...
	for i in 0..n {
		let foobar = libm::floorf(bark(i as f32 * hfl_dn) * foobar_const_part);
		let map_elem = foobar.min(bms_m1);
		let cos_omega = libm::cosf(map_elem * omega_factor);
		res.push(cos_omega);
	}
	return res;
}
