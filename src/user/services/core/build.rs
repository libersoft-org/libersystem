...
use system_manifest::{DriverLifecycle, Manifest, MatchPriority, Presence, RoleKind, Stage};
...
fn generate_driver_registry(manifest: &Manifest) {
	...
	// Publication allowances apply to each binding, so a program count cannot bound instances.
...
fn generate_services(manifest: &Manifest) {
	write_generated("manifest.rs", &system_manifest::service_manifest_source(manifest));
