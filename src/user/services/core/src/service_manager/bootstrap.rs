...
pub(super) fn deliver_roles(manager_side: u64, index: usize, kept: &mut Kept, external: &mut dyn FnMut(&Role) -> Option<(alloc::vec::Vec<u8>, u64)>, follow: &mut dyn FnMut(&Role) -> Vec<u64>) -> bool {
	for (slot, role) in ROLES[index].iter().enumerate() {
...
pub(super) fn drive_runtime_drivers(dm_control: u64, storage_client: u64, net_online: &mut bool, gpu_online: &mut bool, snd_online: &mut bool, input_online: &mut bool, usb_online: &mut bool, usb_pointer_online: &mut bool, raw_keys: &mut u64, buf: &mut [u8]) {
	if dm_control == 0 {
		return;
	}
	...
	for _ in 0..8 {
		...
		match tag {
			...
			// whether this machine has a sound driver bound, which is the only thing this
			// supervisor ever did with that handle. A handle under it would be one nobody serves.
			_ if tag.starts_with(b"SND") => {
				*snd_online = tag.len() > 3 && tag[3] != 0;
				if handle != 0 {
					close(handle);
				}
			}
...
pub(super) fn console_report(source: &[u8], event: &[u8]) {
...
pub(super) fn stop_service(control: u64, up: u64, buf: &mut [u8]) -> State {
...
fn bootstrap_permission_manager(manager_side: u64, policy_admin: u64, font_root: u64, font_admin_root: u64, storage_admin: u64, storage_client: u64, media_client: u64, iso_client: u64, udf_client: u64, usb_client: u64, ram_client: u64, tmp_client: u64, catalogue_root: u64, log_client: u64, net_client: u64, time_client: u64, config_client: u64, device_client: u64, audio_client: u64, display_admin: u64, input_admin: u64, audio_admin: u64, resource_client: u64, process_client: u64, session_client: &mut u64, session1: &mut u64, perm_client: &mut u64, admin_server2: &mut u64, stats_server2: &mut u64) -> bool {
	...
	// AND THE OPERATOR'S WRITE, held apart from that read.
	//
	// A tool granted `DEVICE` renders a device list; one granted this CHANGES a binding, and
	// nothing holding the first is any closer to the second. This is the only path to the four
	// verbs: DeviceManager serves them on one endpoint, this supervisor holds its far end
	// solely to hand it here, and PermissionManager mints the operator's connection from it.
	//
	// Sent whether or not there is one - the read is positional, and a boot that minted none
...
pub(super) fn bootstrap_serve(manager_side: u64, client: &mut u64) -> bool {
	serve_root(manager_side, CAP_SERVE, false, client)
}
...
// choice.
pub(super) fn serve_root(manager_side: u64, tag: &[u8], handed_on: bool, client: &mut u64) -> bool {
