...
pub unsafe fn negotiate_features(mmio_base: u64, info: &DeviceInfo, want_word0: u32) -> Option<Virtio> {
	unsafe {
		...
		// AND `ACCESS_PLATFORM` WHERE THE DEVICE ASKS FOR IT. A device behind an IOMMU offers this
		// bit and will not accept `FEATURES_OK` without it: acknowledging it is the driver saying
		// the addresses it programs are the platform's rather than raw physical ones - which is
		// exactly what they are, since `SYS_DMA_BUFFER_PHYS` answers a translated device with the
		// IOVA the kernel can revoke. Taken only when OFFERED, so a device with no IOMMU in front of
