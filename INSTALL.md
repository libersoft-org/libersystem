...
This builds a bootable Limine ISO from the kernel ELF and launches QEMU headless, with the kernel's serial console wired to your terminal. The boot log ends in `boot OK` and the kernel drops into an interactive serial shell at a `liber>` prompt. Type `help` to list commands (`ls <vol://volume>`, `cat <vol://vol/path>`, `graph`); type `exit` to stop the shell, after which the kernel idle-spins. To quit QEMU at any time, press <kbd>Ctrl</kbd>+<kbd>A</kbd> then <kbd>X</kbd>.
