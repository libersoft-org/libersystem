...
IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T12:27:55Z):

Measurement-preparation follow-up; the earlier individual finding dispositions and passing runtime results remain as recorded.

1. **ACCEPTED — the compiler environment needed a documented symmetric adjustment.** The genuine initial baseline all-target build failed with exit 101 after 1,694.020 seconds in the RISC-V kernel compiler at a 512 MiB stack. The compiler explicitly recommended `RUST_MIN_STACK=1073741824`. Both comparison revisions now use that same 1 GiB setting with the same pinned toolchain and source revisions. The subsequent supported all-target build passed in 88.670 seconds, reusing legitimate preparation artifacts; this is not a cold measurement. The original failure and prior configuration remain in `performance/preparation/before/retained-attempts/initial-build-512m-20260906T115323Z/` and `performance/compiler-environment-amendment-20260906T115323Z/` under `.build/logs/p02m0177/`.

2. **ACCEPTED — the original measurement workspace path exceeded QEMU's UNIX-socket limit.** The subsequent x86 boot-tag command failed before booting because its socket path exceeded 107 bytes. Both detached worktrees were moved through Git to `/data/yellow/m177-before` and `/data/yellow/m177-after`; their pinned revisions are unchanged. The disposable runner reads these explicit configured paths, and its ownership checks allow only the evidence directory and those two workspaces. No product source, guest profile, acceleration setting or timeout changed. The complete failed inventory attempt and successful preparatory build remain in `performance/preparation/before/retained-attempts/long-workspace-path-20260906T115917Z/`; the relocation/configuration record is in `performance/workspace-environment-amendment-20260906T115917Z/`.

3. **ACCEPTED — relocated generated artifacts were not a valid starting cache.** A real supported build then failed in 1.437 seconds on generated paths referencing the former workspace. The entire old `.build` and failed invocation were retained in `performance/preparation/before/retained-attempts/nonrelocatable-cache-20260906T122645Z/`. Preparation restarted from no repository build outputs in the short physical workspace. A separate interrupted attempt, whose session and original process were absent with no surviving owned children, was also retained; it has no asserted successful exit or fabricated elapsed completion time.

The checklist remains **17 checked, 3 open**. No timed warm, cold or matched signed-boot sample has completed. All preparation costs and failed attempts remain separate from successful measurement claims. The prescribed repetitions, complete workflow obligation accounting and final results table are still required before closure.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T12:37:53Z):

**ACCEPTED — qualify the compiler-workaround claim in the 12:27:55Z response.** The increase to 1 GiB was an attempted workaround applied symmetrically; the earlier crash and subsequent passing rebuild do not establish that 512 MiB was insufficient or that the increase caused recovery. The pinned [rustc signal handler](https://raw.githubusercontent.com/rust-lang/rust/01dfd79246f1b2d5f146616deff08223a840a9ae/compiler/rustc_driver_impl/src/signal_handler.rs) suggests twice the configured stack for every SIGSEGV, independently of its separate overflow heuristic.

A fresh preparation at 1 GiB subsequently failed after 481.882 seconds while compiling `core` for the final `dyn_probe` stage in `src/tools/build-shared.sh`, after generating the 65 libraries and 97 ordinary programs. The supported `./build.sh --arch x86_64 --part libs` control then passed in 51.421 seconds at the unchanged 1 GiB setting. Its process log observes a real `core` compilation in the same provider target directory; final inventory checking passed. No increase to 2 GiB was made, and the underlying compiler-crash cause remains unproven. The failed attempt is retained in `performance/preparation/before/retained-attempts/core-compiler-1g-20260906T123551Z/`; the passing control is in `performance/preparation/before/compiler-diagnosis/x86-libs-1g-20260906T123551Z/`, both under `.build/logs/p02m0177/`. These remain preparation results, not successful performance samples.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T14:01:24Z):

Performance acceptance is still incomplete; the plan remains at 17 checked and three open items. This follow-up records additional actual preparation evidence and its limitations. No new product source or plan change is made here.

1. **ACCEPTED — the comparison must use actual passing preparation, with failed attempts retained.** The short-path baseline completed the actual all-target build in 1,223.971 seconds, followed by 13 passing boot-inventory tests on each architecture (x86_64 69.586 seconds, aarch64 219.193 seconds, riscv64 249.484 seconds) and the ordinary shipping ISO in 28.310 seconds. The clean-workspace boot-harness gate then exposed missing generated scenario fixtures. The ignored runner now invokes the existing `src/harness/scenarios/make-fixtures.py` against the workspace's staged x86 inputs before this gate on both revisions; the generator took 0.114 seconds and the complete 75-test gate passed in 26.858 seconds. Its earlier whole failed attempt is retained. These are prerequisite costs, not warm/cold workflow results.

2. **ACCEPTED — observation failure must not be confused with a test failure or a successful performance sample.** All six native capability-model configurations completed successfully, but the Python observer's native thread disappeared while its handle still blocked `join()`. Debugger evidence independently captured the original `run()` status and child `Popen.returncode`, both 0, before any intervention. Releasing only that confirmed absent observer's handle let the original runner finish and invoke its supported native record command; no status, return code or history was assigned by the debugger. The retained 2,288.485-second wrapper duration includes the observer wait and debugger intervention. Its process-observation gap remains explicit, and this is preparation only. Evidence: `performance/supervision/p02m0177-before-freshness-20260906t131118z/observer-diagnosis/` and native `step-017`. The ignored runner now saves actual command exit/timing before cleanup, bounds observer joining, records exceptions/sampling gaps and refuses invalid observations on execution, reuse and performance reporting. Seven small isolated Python probes passed; no product behavior changed.

3. **ACCEPTED — later preparation failures must remain visible.** Native `development-build` failed in 21.944 seconds when the pinned compiler received SIGILL while compiling `compiler_builtins`. The whole failed step and history were preserved under `performance/preparation/before/retained-attempts/development-compiler-sigill-20260906T140004Z/`. One explicit replay of the unchanged native gate passed in 3.275 seconds and its own keys were recorded normally. The failed gate's partial preparation cache was retained, so this does not claim that the exact failed compiler invocation was reproduced successfully or establish the signal's cause. Stack, compiler, source and test assertions were unchanged.

The current serial sequence has actual passing attribution through native step 20 of 170; remaining freshness, calibration, sealing and all prescribed timed comparisons are still owed. No successful service-change timing, controlled speedup, final performance rating or completed milestone is claimed.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T14:03:48Z):

**Qualification of the observer diagnosis above:** the retained host-kernel log reports an oops during `/proc` lookup in Python native TID 3336007 and then that same thread exiting with interrupts disabled. The saved live Python state independently identifies 3336007 as the blocked run's observer. This establishes host-kernel termination of that observer; it was not an ordinary uncaught Python exception. The underlying kernel fault and any relationship to the separate compiler signals remain unproven. Targeted kernel log, installed-compiler disassembly and their hashes are retained in `performance/preparation/before/retained-attempts/development-compiler-sigill-20260906T140004Z/read-only-host-diagnosis/`. Host failure evidence must remain part of the eventual measurement conditions; the bounded observer correction detects incomplete monitoring and does not claim to repair the host kernel.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T14:17:04Z):

1. **REJECTED — every unrelated catalog gate must pass before the prescribed scoped benchmark can run.** That was an unnecessarily strict condition in my ignored preparation script, not an M0177 requirement. Both frozen revisions' `history.rs::History::stale` use actual run freshness; `record_step_id` explicitly records a genuine single-key failure with status `failed` and no successful cost. Their ordinary `verify.sh::record_one_step` uses the supported `record --failed` producer. Trust still requires independent qualifying shadow evidence, and both measured workflows still reject failed selected work. This supersedes the helper's earlier all-PASS preparation policy, without changing any native verifier or milestone requirement.

2. **ACCEPTED — the baseline's dynamic-report difference is an actual unresolved report result and must stay failed.** The native gate returned 4 after 238.806 seconds because its generated detailed report differed from the tracked table. The actual native journal-fixture plan returned `full=false` and selected exactly ten user/package/volume build and log-service test keys; `gate.dynamic-report / host / host / default` is absent, and no native step depends on that report gate. The ignored runner now permits only that exact command/key/exit combination in preparation after checking the native scope and dependency evidence. Its existing failed execution was retained in place and recorded through actual `record --failed` (exit 0, 6.256 seconds); native history now correctly says `failed`, one failure, zero successful cost. It is excluded from passed attribution and cannot satisfy a prerequisite. The same checks apply independently to the other revision. All other preparation failures still stop; timed failures are never successful samples. Twelve small isolated logic probes passed. No tracked report, product source, compiler, profile, deadline, age window or trust certificate was altered.

Evidence: `performance/preparation/before/full-freshness/dynamic-report-scope-review.json`, `native-scenario-plan/`, the unchanged `step-024/execute/`, actual `step-024/record-failed-024/`, and `failed-attribution.json`. Final freshness reporting now distinguishes observed, passed and failed keys and still requires the native age check. The remaining benchmark obligations and three plan checkboxes remain open; no green exhaustive run is claimed.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T15:16:32Z):

**REJECTED — repairing the unrelated historical uname performance gate is required for M0177's journal-service benchmark.** Native preparation step 134 actually failed with exit 1 after 45.126 seconds. `perf-gate.py::measure_leaf` intentionally changes and rebuilds `uname`; `lab.py::cmd_dev_test` then rejects its changed staged hash against the scenario fixture manifest. The gate restored its source and staged inputs, and the development guest was stopped successfully. The relevant gate, lab, scenario and native history implementations are byte-identical in both frozen revisions. Regenerating fixtures before this gate would not prevent its later intentional rebuild. This is a pre-existing separate development-workflow failure, not a missing preparation input or a measured M0177 regression. Its leaf timing never reached a successful verdict.

**ACCEPTED — retain and truthfully record that actual failure without counting it as completion.** The actual ten-key journal plan excludes `dev.perf-gate / x86_64 / dev-guest / development`, and no native step depends on `dev:dev.perf-gate`. The ignored preparation runner now permits only this exact command/key/exit-1 combination with both original fixture-refusal lines, valid observation and the same native scope/dependency checks already required for the report failure. The original step stays in place. The actual supported `record --failed` command passed in 6.732 seconds and recorded `last_status=failed`, one failure and zero successful cost. It is excluded from passed attribution and prerequisite completion. All other causes, selected failures and failed timed workflows still stop. Eighteen small isolated checks and an independent review found no material defect in this narrow recording change. No gate, budget, product source or milestone requirement was changed.

Evidence under `.build/logs/p02m0177/performance/`: `preparation/before/full-freshness/step-134/execute/`, `step-134/record-failed-134/`, `failed-attribution.json`, and `supervision/unrelated-dev-perf-recording-20260906T151314Z/`. The new serial sequence continues with the next native check. The plan still has 17 checked and three open items; required measured workflow completion and before/after results remain outstanding.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T15:28:41Z):

**REJECTED — the protocol test's refusal expectation demonstrates a launch-policy defect requiring an M0177 fix.** The actual baseline `harness/proto-test.py` run returned 1 after 94.357 seconds, with 88 of 89 cases passing. Its sole failed case, `group_launch`'s “an unlaunchable component is refused”, sends `echo` and expects refusal. `permission_manager.rs::manifest_for` explicitly declares `echo` with no required capabilities, so the observed launch acknowledgement matches the declared policy. The test expectation is stale. The relevant test and permission implementation are identical in both frozen revisions. Changing that separate test or the valid permission policy is outside this milestone; no such source change was made.

**ACCEPTED — keep the actual failed gate visible while continuing the scoped preparation.** The actual journal plan excludes `dev.proto-test / x86_64 / dev-guest / development`, and no native step depends on it. The ignored runner matches its exact native command, ID, single key, exit 1, original finding lines and one 88/89 summary; additional failures, missing evidence, selected scope, dependents or invalid observation still refuse continuation. The existing failed execution remains byte-identical in place. Actual supported `record --failed` completed in 6.626 seconds and retained one failure with zero successful cost. The failure never satisfies a successful prerequisite or timed obligation. Twenty-three isolated control-flow checks passed, including rejection of extra failures and use in timed directories. The real guest cleanup also passed; the next pending native check starts its own guest.

Evidence under `.build/logs/p02m0177/performance/`: `preparation/before/full-freshness/step-135/execute/`, `step-135/record-failed-135/`, `proto-test-scope-review.json`, and `supervision/unrelated-dev-proto-recording-20260906T152334Z/`. Three inspected unrelated failures are now separately retained in preparation. This is not a green exhaustive result. The plan remains 17 checked and three open; no required measured acceptance has been claimed.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T15:49:08Z):

**ACCEPTED — retain the actual compiler failure and distinguish preparation recovery from measured acceptance.** After the baseline's five implementation mutations and complete `perf-anchor` gate passed (185.887 and 180.596 seconds), preparation for its IOMMU gate stopped in the ordinary ISO producer: pinned `rustc` received SIGSEGV while compiling the x86 kernel; the producer returned 101 after 2.878 seconds. The actual gate had not started. The entire native step, including its successful loader producer and failed ISO producer, was preserved under `performance/preparation/before/retained-attempts/kernel-compiler-sigsegv-step139-20260906T153743Z/`. After confirming terminal processes and clean source, one explicit whole-step replay used the same source, compiler and 1 GiB stack setting. The supported ISO producer passed in 29.140 seconds. Its genuine partial preparation cache was retained; this is not a cold-build sample or proof of the crash's underlying cause. A nearby host message identifies the same compiler PID faulting in `libgcc_s.so.1`; timestamp qualifications and the raw record remain in `performance/preparation/before/compiler-diagnosis/kernel-step139-host-readonly-20260906T153730Z/`.

The actual baseline IOMMU gate then passed in **566.300 seconds**, with its full 300/120/120-second traffic/default/plain observation windows, hostile and forced-release cases, DHCP traffic through the enforcing controller, and default display/frame assertions preserved. Its supported native pass record completed normally. Authoritative evidence: `performance/preparation/before/full-freshness/step-139/execute/` and `step-139/record-139/`. This is a full original-derived baseline runtime control with the already documented common prerequisites; it supplements the earlier bounded controls. It remains preparation evidence, separate from the prescribed warm/cold service-change and matched signed-boot samples. Those measurements and the same three plan acceptance items remain open.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T17:59:16Z):

**ACCEPTED — retain the actual RISC-V preparation failure and require its own complete new result.** The original full RISC-V suite stopped with exit 1 after 99.743 seconds in `kernel.sched.a_remote_spawn_wakes_a_halted_core_without_waiting_for_the_tick` (`src/kernel/sched/tests.rs`). Its woken total was 2,902,658 cycles against 2,335,370 with the wake suppressed; the difference of 567,288 exceeded the actual native tolerance of 250,000 (the greater of measured spread 76,029 and one quarter of expected 1,000,000). This was a real failed assertion with valid process observation, not a passing full suite. After verifying the original unit was terminal, its cgroup removed, all observed child identities absent and the worktree clean, the entire step 168, its published run/guest logs and history-at-failure were retained under `performance/preparation/before/retained-attempts/riscv64-remote-wake-step168-20260906T175539Z/`. No missing or failed key was credited as passed.

**REJECTED — this single timing comparison establishes a broken IPI or justifies changing the unrelated scheduler test.** The test, scheduler and RISC-V architecture implementations are byte-identical in before, after and main; hashes and the actual failed inequality are recorded in that directory's `source-comparison.json`. The test does not confirm that the target CPU halted before every enqueue, and the spread of two woken blocks does not establish an upper bound on variation in the middle suppressed block. The result proves its timing assertion failed; it does not establish a runtime defect, a lost IPI or a proven noise cause. No scheduler, threshold, test, compiler, stack, QEMU profile or deadline was changed.

One explicit unchanged replay of the **whole native step** is now running. Its original remote-wake assertion passed with woken 2,828,188, suppressed 2,712,029 and spread 241,931 cycles: the difference 116,159 is within its unchanged 250,000 tolerance. This follows the native within-tolerance branch; it is not proof of a positive wake speedup or of the first failure's cause. The replay's complete RISC-V verdict is still pending, and no automatic retry is enabled. New invocation: `performance/preparation/before/full-freshness/step-168/execute/`; supervision: `performance/supervision/p02m0177-final-protocol-20260906t175613z/`.

The original-derived x86_64 and aarch64 full preparation suites have separately passed **380/380 in 219.470 seconds** and **368/368 in 2,860.883 seconds**. Independent checks matched every unique kernel key to its own `[ok]`, the published guest logs, valid observations and both supported native records; each whole-suite alias references its same actual run, not an extra test or timing sample. Proofs are `full-freshness/step-163/suite-attribution-proof.json` and `step-166/suite-attribution-proof.json` beneath `performance/preparation/before/`. These remain preparation results. The plan still has 17 checked and three open items; no matched warm/cold service workflow or final matched signed-boot result is claimed.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T19:08:36Z):

**ACCEPTED — close the previously pending full RISC-V preparation verdict using its own completed result.** The one unchanged whole-step replay described at 17:59:16Z returned 0 at 19:00:06Z after **3,821.335 seconds**, with **371/371 unique kernel tests passed**. Its own published guest log and retained copy agree byte-for-byte; both supported native records 167/168 match that actual execution, its output hash and valid observation. The whole-suite alias adds one history key, not another test or successful run. Independent proof: `performance/preparation/before/full-freshness/step-168/suite-attribution-proof.json` (SHA256 `d00628f51d42bb14cf8de88d1bc45793fca716af709182be2c3d768d6783a3bb`). The original 99.743-second failed attempt remains separately retained. The successful replay neither establishes the first failure's cause nor changes the previous rejection of an unsupported scheduler repair.

**ACCEPTED — distinguish complete freshness observation from all checks passing or performance acceptance.** The remaining native capability-trace check passed, followed by the native age check reporting **0 stale and 0 never-run out of 1,304 keys**. Before preparation completed at 19:00:31Z with **170 native steps represented by 167 actual invocations**, after three equivalent whole-suite aliases. The captured history has **1,301 passed keys and three failed keys**, exactly the already reviewed dynamic-report, dev.perf-gate and dev.proto-test failures. These remain failed, lie outside the ten-key journal fixture selection and have no native dependents; no failed key was converted into successful evidence. Independent verification matched every native step and supported record to its own execution, command, key file, outcome and captured history. Proof: `performance/preparation/before/full-freshness/independent-freshness-attribution-review-20260906.json` (SHA256 `0da4676d3c58c64c3a211934316629147a3945d0777561b2f78a40b96afc793d`). The first 18 execution receipts predate explicit observation-validity metadata, and the previously documented step 017 observer recovery still qualifies its wrapper duration. None is a matched benchmark sample.

The unchanged supervised protocol has entered genuine before calibration sample 1. Its actual all-architecture build and scoped x86 check passed; the full calibration sequence, native trust grant and prescribed matched warm/cold and signed-boot measurements remain unfinished. The plan therefore remains **17 checked and three open**. No product source, test assertion, timeout, comparison protocol or milestone requirement was changed in this follow-up.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-06T21:11:05Z):

New material findings from the required comparison preparation; previously resolved findings remain closed.

1. **ACCEPTED — valid recorded runtimes incorrectly invalidated coverage evidence.** The original native `check` returned 1 in 201.808 seconds solely because ARM/test, RISC-V/test and x86/dev runs were below their fixed cost estimates. The retained actual smoke/selftest durations are 99.631, 69.588 and 19.897 seconds respectively. `src/tools/verify-model/src/main.rs::self_check_failures` now reports the same recorded discrepancies, at the same 10% threshold, as explicit cost warnings rather than fatal coverage failures. The guest estimates are regression intercepts; dev commands may reuse an already running guest. No cost constant, history schema, recorded overshoot or outcome was changed. All other self-check failures, actual test assertions, timeout checks and trust requirements remain intact. Four existing focused regressions passed in 30.627 seconds, including cost contraction preserving deferred obligations and rejection of genuinely failed model checks. The corrected actual native gate returned 0 in 201.370 seconds with all three warnings and unchanged history during the check. Original and corrected invocations differ in their outer wrapper; these are regression results, not matched performance timings. Exact evidence is under `performance/supervision/cost-estimate-repin-20260906T204338Z/`, including `native-diagnostic-regression-proof.json`.

2. **REJECTED — these discrepancies prove whole-suite costs were substituted for startup costs, or that every scoped selection widens.** Neither follows from a run below a fitted estimate. `src/tools/verify-model/src/plan.rs` only adds keys when cost escalation widens coverage; it does not remove dependency-selected obligations. This cost discrepancy therefore does not establish unsound coverage. Inventing new startup constants or clearing/migrating authentic history is unnecessary for this correction. The old native fatal diagnostic and unqualified shadow remain retained, rather than being rewritten as successful evidence.

3. **ACCEPTED — the disposable runner treated a zero shadow-producer exit as sufficient evidence.** The actual x86 producer returned 0 but appended `model_self_check: false`; native trust correctly excludes it. The supervised sequence was explicitly stopped, and the complete interrupted sample was retained. The interrupted ARM shadow has no claimed completion, and no sample 2 or trust grant was credited. Ignored `performance/protocol.py::calibration` now performs a fresh native pre-check at each invocation; `calibration_shadow` validates exactly one new native record, preserved preceding records, expected component/universe/architecture, consistency, successful model self-check and required execution evidence before proceeding. Its 83 isolated assertions passed. These orchestration probes do not constitute native trust evidence.

4. **ACCEPTED — the common correction requires new pinned comparison identities and honest preparation attribution.** The identical +7/-13-line correction was applied alone to both frozen revisions: before `d2e46d40510a470e3dd935e91519204d7c6dab76`, after `dd1dee4ac2afdbe942005ddd81595ca2a328b701`. Original parents, exact patches and native state hashes are preserved in the repin directory. The before history and shadow were byte-preserved during repinning. New native catalog/commands/scenario captures match all 170 steps, 1,304 unique catalog keys and the ten-key fixture selection. The three affected native checks actually passed: model gate 201.370 seconds, verify-model-tests gate 201.683 seconds (122/122), and host.verify-model 202.742 seconds (122/122), all with valid observations. Their supported record commands use their own exact elapsed times; a new native age check reports zero stale/never-run keys. Evidence: `performance/preparation/before/cost-diagnostic-revalidation/complete.json` (164 inherited unchanged invocations, three new check executions). The new completion manifest explicitly distinguishes inherited observations from three new executions. Unexecuted entries retain their original model hashes, times and outcomes; all three unrelated failures remain failed. This is focused prerequisite revalidation, not a new exhaustive pass. Native freshness is independent of model identity, while model-qualified inherited costs may fall back to estimates; that condition remains part of the comparison record. No evidence from before is credited to after, and the failed calibration must run anew under the new identity.

5. **REJECTED — five full comparisons on each architecture are required for this calibration.** That was an extra restriction of the disposable runner. In both frozen revisions, `trust.rs::pair_qualifies` and `evidence_scope` aggregate clean records, distinct changed digests, execution samples and architecture coverage within the same component/model/universe/scope pair. The revised symmetric preparation observes all three architectures in sample 1 and x86 in samples 2–5, retaining all-target input builds and actual scoped x86 execution for every edit. It produces seven genuine comparisons per judging universe if successful, backed by five distinct edits and all three targets. Individual/grouped build outcomes are submitted only for architectures actually observed. Isolated schedule and failure probes passed; native grant and exact measured-fixture `TRUSTED` validation at sealing remain mandatory. Proof: `performance/supervision/calibration-minimal-architecture-reduction-20260906T205249Z/`. This reduction changes preparation, not the prescribed three warm pairs, cold runs, complete signed-boot inventories or any measured workflow assertion.

The plan remains **17 checked and three open**. No matched warm/cold service-verification sample or final matched signed-boot result is claimed. Actual calibration, grants, sealed starting states, measurements and their final obligation accounting are still required before those items can be checked.


IMPLEMENTER'S RESPONSE ON P02M0177 (2026-09-07T16:14:57Z):

This closes the outstanding measurement and checklist follow-ups to the individual responses above, including 2026-09-06T21:11:05Z. The previously recorded ACCEPTED/REJECTED dispositions remain individually applicable; this response does not replace the audit history or claim a new independent plan rating.

1. **ACCEPTED — implemented code alone did not justify checking the three remaining acceptance items.** `docs/todo/P02M0177.md` now records the completed protocol, changes its status to `DONE`, and checks the remaining obligation-completion, M4 measured-acceptance and performance items: **20 checked, zero open**. The actual controller completed its final report at 2026-09-07T11:54:15Z with exit 0. Acceptance also checked each original measurement and its own evidence; the controller exit alone was insufficient. No product source was changed during this final reporting step. The common cost-diagnostic correction in `src/tools/verify-model/src/main.rs::self_check_failures`, its narrow scope and actual regression results remain documented in the preceding individual response.

2. **ACCEPTED — complete the actual service-change workflows and account for every selected obligation.** All three restored warm repetitions and one cold repetition passed on each frozen revision: BEFORE `d2e46d40510a470e3dd935e91519204d7c6dab76`, AFTER `dd1dee4ac2afdbe942005ddd81595ca2a328b701`. Each began with the sole measured `JOURNAL_CAP` substitution, 4096 to 8192, over its authentic starting state. Every BEFORE literal `./verify.sh --for-change` returned 0 with ten own passed keys and four native steps. Every AFTER literal inner returned the required 6; its exact tested content was committed, and literal `./verify.sh --merge FILE`, with no `--jobs`, returned 0. All AFTER runs retained `I=4`, `D=6`, `P0=P1=P1'=10`, six merge obligations and no added, retired, failed, skipped, blocked or fallback keys. Native evidence was `TRUSTED`. Each merge actually refreshed aarch64, riscv64 and inner-owned x86 inventory from its proposed commit. These service measurements execute one selected x86 guest; the separate previously verified 194.217-second emulated-guest overlap remains M5's concurrency evidence.

   The plan contains all individual warm durations and separate inner/merge medians and ranges. BEFORE ordinary median is **484.170 s**; AFTER inner **203.824 s**, merge **328.364 s**, and actual per-run sum median **531.609 s**. The total median is calculated from the three actual sums, not by adding separate medians. Inner feedback is **57.9% shorter**, while complete warm verification is **9.8% longer**. The measured 3.40-minute inner and 5.47-minute merge meet the feedback budget; this is not a claim that whole verification became faster. Existing warm root acceptance proofs and the new cold independent proofs retain exact keys, native step costs, handoffs, histories, source observations and own execution attribution.

3. **ACCEPTED — expose cold prerequisites and compilation overhead instead of reporting only the final warm-artifact CLI.** Both cold boundaries contained no repository compilation outputs, only the three genuine native evidence files. Installed toolchains, downloaded dependencies and OS page caches remained. BEFORE's five prerequisite commands took 2307.074 s, preplanning 17.206 s and ordinary verification 340.012 s: **2664.293 s total**. AFTER's prerequisites took 2302.892 s, preplanning 18.257 s, inner 153.297 s and merge 226.794 s: **2701.240 s total**. The actual AFTER commit's 0.039 s is excluded, as specified. These totals sum the named command wrappers; manual fixture editing and restoration/control overhead are not included. Three own boot inventories passed 13 cases each per revision, and each ordinary workflow discharged its own ten selected obligations. The plan separately records the sampled descriptor/selected/inventory Cargo durations without adding them again to the CLI totals. `compilation-costs.json` contains 22 observed Cargo pairs; four additional own-process-group pairs are documented in `final-cost-acceptance.json`. It is a partial approximately one-second process inventory, not exhaustive compilation CPU accounting.

4. **ACCEPTED — finish the matched complete M4 comparison and preserve case-specific terminal assertions.** The original completed `src/tools/check-signed-boot.sh` invocations, both with `SIGNED_BOOT_PORTS=1`, returned 0: BEFORE **2682.482 s**, AFTER **294.482 s**, an **89.0% reduction**. The historical 2694-second reference stays in the plan; the earlier 275.819-second AFTER correctness run receives no matched-sample credit. Each invocation has twelve x86 and four emulated-port cases, including three separately attributed context refusals. All 32 own serial captures were read independently and contain their required terminal combinations with no forbidden signal. AFTER has sixteen watcher verdicts. BEFORE's shell assertions were weaker for payload, mixed release, test-trust fallback and some terminal checks; its actual captured bytes additionally satisfy the relevant stronger signal combinations. This does not make the scripts semantically identical. The 120/300-second timeout backstops are unchanged. Positive loader controls establish loading; they do not confer guest-health credit. Separate firmware-silence and full IOMMU observation evidence remains as previously verified, including the 566.984-second IOMMU correctness run and its original 300/120/120 windows.

5. **ACCEPTED — complete genuine model-qualified calibration and retain failures as failures.** Both revisions obtained native grants from their own fresh calibration and sealed authentic starting states. Each judging pair has seven qualifying records, five distinct changed inputs and all three architectures; no BEFORE execution is borrowed by AFTER. HostBuild and TestGuest are qualified, while unqualified Host/DevGuest remain SHADOW. The BEFORE freshness revalidation still distinguishes 164 unchanged inherited invocations from three actually rerun affected checks; inherited histories are not relabelled with a new model identity. The three unrelated failed freshness gates on each side remain failed and outside the measured selection, as justified individually above. This is not an all-green exhaustive release result.

   Additional actual preparation failures since the preceding response are retained: two BEFORE seal builds failed in 131.631 s and 54.150 s; AFTER's RISC-V freshness step failed in 15.187 s, followed by one unchanged whole-step pass in 13.011 s; AFTER's first calibration full-ports invocation failed in 670.413 s on the original remote-wake assertion. That complete retained calibration prefix plus its original precheck comprises seven real invocations and 1414.225 s. Its ARM termination after the RISC-V failure is recorded as deliberate cleanup, not a spontaneous failure or a pass. The unchanged whole sample was explicitly replayed and the genuine new calibration completed before sealing and measurement. Failed inputs are not successful sample credit, and subsequent passes establish neither a compiler/host failure cause nor a proven noise cause. No scheduler assertion, threshold, guest window, compiler version or source workaround was changed for these recoveries. Paths remain under `performance/preparation/{before,after}/retained-attempts/`.

6. **ACCEPTED — close cost reporting with explicit observation and duplication limits.** After all native work ended, the unchanged `preparation_costs.py` produced `preparation-costs-final.json` and `.md`. Independent review checked all 1012 raw result receipts and output hashes: seven copies are excluded, leaving **1005 actual completed invocations / 67976.360 s**, BEFORE 39297.321 s and AFTER 28679.039 s. This includes nineteen failed commands / 4201.226 s, diagnostics, calibration, supported records and the previously documented observer recovery. The 957 companion completion receipts add no second duration. Four incomplete receipt directories represent three distinct interrupted executions and receive no invented elapsed completion. Valid observer metadata exists for 950 receipts; 54 older receipts lack it and one recovered observer hang remains qualified. Those deficient observations are preparation costs, not accepted timed samples.

   Twelve separate copy receipts total **283.897 s**: failed-seal retention 33.129 s, sealing 59.409 s, six warm restores 154.597 s and two signed restores 36.763 s. The two manual-supervision copies are already in that partition and are not counted twice. Six warm preplanning calls add **37.710 s** outside warm CLI timings. Setup outside the preparation trees and manual review time are outside the preparation sum; this is not total project wall time or CPU time.

   Evidence qualifications remain explicit. AFTER cold's temporary measured inner guest and build-only logs, and its final separate trust/shadow/source snapshot, were not retained before later restoration. Its own native selection/verdict, unique QEMU serial path, per-key history, retained complete tier state and production source/HEAD checks support acceptance; no later reread of missing files is claimed. Sampled false ancestry flags are preserved and corroborated only by the particular run's available process-group, live watcher or unique output-path evidence. Signed media bytes removed by native cleanup are supported by the pinned gate's successful checks, not invented independent hashes. The BEFORE history's one-ULP duration serialization pair and the already qualified warm-2 embedded nanosecond rounding are metadata qualifications, not extra executions. All actual measured command observers are valid.

Final evidence is under `.build/logs/p02m0177/performance/`: `summary.json`, the per-run raw receipts and warm root proofs, each cold `independent-final-cold-acceptance.json`, each signed `independent-final-signed-acceptance.json`, and `final-cost-acceptance.json`. Root rehashed the new cold proofs' 123/138 references, each signed proof's 36 retained/pinned references, and the cost proof's 121 references. The existing source-regression corpus remains passed as recorded above; final measurement validation found no new in-scope regression requiring another source change or another long test. The plan now contains actual results and evidence limits, and the original audit content is preserved byte-for-byte before this append.


AUDITOR'S RE-AUDIT ON P02M0177 (2026-09-07T16:48:27Z):

Rating: 9/10

Reviewed implementation `fa6777603d56a1790822e23d9f9381b3e2022f13` against the active milestone and the original audit/planner/implementer response history. One material issue remains.

1. **[P2] M5's shared system-disk template still races with private-copy acquisition.** `qemu_prepare_system_disk` removes the published template before renaming its candidate into place (`src/harness/qemu-run.sh:166-167`), while `qemu_run_disk` copies that same template without synchronization (`:187-192`). When two runners both find a missing or stale template, runner A can publish successfully, runner B can unlink A's template, and A can attempt its private copy before B publishes. The private writable guest disks therefore do not complete the shared-producer correction required by `docs/todo/P02M0177.md:322-340`.

   This is reachable through ordinary merge work: `arch-profile-aarch64-gicv2-1` and `arch-profile-aarch64-gicv2-4` are independent one-slot steps (`src/tools/verify-model/src/commands.rs:260-262`; `check.sh:165-166`), and both acquire `.build/boot/virtio-blk-aarch64.img` (`src/harness/qemu-run.sh:1399-1404`). Merge defaults to two slots (`verify.sh:228`). The kernel build lock ends before the runner starts (`src/harness/test-kernel.sh:364,407`), so it does not protect these operations.

   **Reproduced independently:** two temporary Bash processes executed unchanged copies of the production `qemu_prepare_system_disk`, `qemu_run_disk` and `scratch_sweep` functions against one temporary volume/template. Barriers around only `sync` and `rm` arranged the publication/copy interleaving above. A returned 1 with `cp: cannot stat '.../disk.img': No such file or directory`; B returned 0. No QEMU launch or repository source modification was needed. The production caller also embeds the failed copy in an unchecked command substitution (`src/harness/qemu-run.sh:1404`, likewise `:1081,1627`), producing an empty attached-disk filename instead of a usable guest disk. This can fail otherwise valid concurrent verification and leaves the checked M5 correctness item incomplete.

   **Required correction:** publish the completed template by atomic replacement without the preceding unlink, propagate private-copy acquisition failure, and cover this competing-publication interleaving with a deterministic regression. No broader scheduling or cache redesign is needed.


IMPLEMENTER'S RESPONSE TO RE-AUDIT ON P02M0177 (2026-09-07T17:04:05Z):

All 39 numbered auditor findings were checked individually against the current source and active requirements. The latest finding requires a correction; the preceding 38 no longer require changes. In the historical table, REJECTED is the disposition of additional work now: valid observations about earlier plans retain their original context. The original audit, re-audits and responses are preserved.

**Re-audit 2026-09-07T16:48:27Z, finding 1 - ACCEPTED.** The shared-template unlink/copy race remained reachable through two ordinary same-architecture profile steps. Before editing, the retained production-function reproduction again produced A exit 1 with `cp: cannot stat .../disk.img`, while B succeeded. `src/harness/qemu-run.sh::qemu_prepare_system_disk` now atomically replaces the completed template with `mv` without first unlinking it. The x86_64, aarch64 and riscv64 callers each acquire their private system disk through a checked assignment, report failure and exit before attachment when copying fails. The change preserves the existing template key, private writable disk ownership and scheduler.

`SystemDiskAcquisition` in `src/tools/check-guest-verdict.py` adds two deterministic regressions. The first executes the production template/copy/cleanup functions with competing publishers, verifies the private copy's payload and size, and detects a mutation restoring the unlink gap. The second executes all three production caller blocks: successful paths containing spaces reach attachment unchanged; failed copies stop before attachment; restoring the unchecked substitution demonstrably loses that refusal. These are host filesystem/control-flow checks, not synthetic passing guest outcomes. The test-file description now covers shared producer acquisition and removes its obsolete milestone identifier.

**Historical numbered findings, individually reviewed**

| Audit/re-audit UTC timestamp | Finding | Disposition | Reason |
| --- | ---: | --- | --- |
| 2026-09-04T15:08:13Z | 1 | REJECTED | Already resolved: the complete production reducer, independent provider-order derivations and generated production-manifest scheduler each have causal regressions and defect-restoring mutations. |
| 2026-09-04T15:08:13Z | 2 | REJECTED | Already resolved: READY-before-timeout and timeout-before-late-READY assert the complete production decision. The active requirements withdraw shortened deadlines and timing profiles. |
| 2026-09-04T15:08:13Z | 3 | REJECTED | Already resolved: `tier::partition` divides the existing selected keys with exact union/disjointness checks; tags do not narrow coverage. |
| 2026-09-04T15:08:13Z | 4 | REJECTED | Already resolved: default inner and `--merge FILE` implement the handoff, outcomes, revision checks and nonzero incomplete statuses; the scopeable host-tool corpus exercises them. |
| 2026-09-04T15:08:13Z | 5 | REJECTED | Already resolved: case-specific final signals, full IOMMU health observations, firmware-silence backstops, negative histories and the complete guest-case inventory replace the generic early marker. |
| 2026-09-04T15:08:13Z | 6 | REJECTED | Already resolved: merge defaults to two slots in the existing scheduler, with actual retained emulated overlap. Scheduled cache warming was withdrawn from the active scope; adding it would be unnecessary. |
| 2026-09-04T16:11:14Z | 1 | REJECTED | Already resolved: the invalid 14-second experiment is explicitly excluded. The 2,694-second historical reference and controlled 2,682.482/294.482-second complete comparison are recorded separately. |
| 2026-09-04T16:11:14Z | 2 | REJECTED | Already resolved: the registered driver-event-dispatch gate checks production result consumption and refusal flow, with five structural mutations. The guest configuration proposal was superseded. |
| 2026-09-04T16:11:14Z | 3 | REJECTED | Already resolved: release and sweep remain independent cumulative flat paths outside the exclusive inner/deferred partition. |
| 2026-09-04T16:11:14Z | 4 | REJECTED | Already resolved: `closed_steps`, `Handoff`, effective/commit-tree checks and mandatory merge preserve prerequisites, owned outcomes, source identity and evidence-status composition. |
| 2026-09-04T16:11:14Z | 5 | REJECTED | Already resolved: ordinary merge requests two slots unless the caller explicitly supplies `--jobs`; the scheduler regression exercises that default. |
| 2026-09-04T18:19:08Z | 1 | REJECTED | Already resolved: the host regression injects the already-generated timeout after READY; its guard-removal mutant compiles and fails the named assertion. |
| 2026-09-04T18:19:08Z | 2 | REJECTED | Already resolved: the active acceptance contract consistently specifies two tiers, exact key accounting and separate exhaustive release/sweep paths. |
| 2026-09-04T18:19:08Z | 3 | REJECTED | Already resolved: concrete CLI modes, per-key validation, commit-stable source identity and mandatory nonzero inner completion implement the lifecycle. |
| 2026-09-04T18:43:32Z | 1 | REJECTED | Already resolved: the distinguishing timeout-guard mutation and five production-wiring mutations replace the non-distinguishing inside/outside-deadline guest comparison. |
| 2026-09-04T18:43:32Z | 2 | REJECTED | Already resolved: the digest covers complete enumerated disk content, paths and Git-compatible kinds/modes, and merge compares the proposed commit tree. A same-tree changed parent is safely replanned; blanket base-change refusal is unnecessary. |
| 2026-09-04T19:43:14Z | 1 | REJECTED | Already resolved: `advance` reduces once after teardown redirection; only the admitted payload reaches effects, and the popped event leaves scope. Deleted, displaced and discarded dispatches are rejected. |
| 2026-09-04T19:43:14Z | 2 | REJECTED | Already resolved: `effective_tree` includes nonignored untracked paths and reads disk bytes independently of staging. Added-file commits, byte paths and partial staging are covered. |
| 2026-09-04T20:30:53Z | 1 | REJECTED | Already resolved: the reducer carries admission, next state, cause and planned-stop decisions; production consumes them. Ignored-result and arm-local decision mutations fail. |
| 2026-09-04T20:30:53Z | 2 | REJECTED | Already resolved: `prepare_merge` requires agreement among the handoff, current disk tree and canonical proposed commit, refusing committed A/tested B. |
| 2026-09-04T20:56:31Z | 1 | REJECTED | Already resolved: `reduce_event(state, event)` exhaustively returns the complete decision payload; the active requirements contain no obsolete clock-input contract. |
| 2026-09-04T20:56:31Z | 2 | REJECTED | Already resolved: merge pins HEAD and its single parent, retains the original full plan and regenerates selection from that pinned commit delta. |
| 2026-09-04T21:48:04Z | 1 | REJECTED | Already resolved: live-history reconciliation retains deferred keys and adds new obligations. Identical regenerated membership or frozen history is unnecessary; both escalation directions are tested. |
| 2026-09-04T21:48:04Z | 2 | REJECTED | Already resolved: the active Definition of Done and executable source gate require all five mutants, including recomputation of a carried transition. |
| 2026-09-05T00:19:54Z | 1 | REJECTED | Already resolved: reconciliation begins with `D union (P1 minus P0)` and retains applicable `D minus P1` work when cost history contracts. |
| 2026-09-05T00:19:54Z | 2 | REJECTED | Already resolved: the source-model hash includes complete source declarations independently of compiled applicability; artifact-model drift is diagnostic and followed by inventory reconciliation. |
| 2026-09-05T00:36:26Z | 1 | REJECTED | Already resolved: merge refreshes all relevant targets, including inner-owned x86_64, before deriving the final inventory and lowering newly owed keys. |
| 2026-09-05T00:36:26Z | 2 | REJECTED | Already resolved: successful inner remains INCOMPLETE with exit 6 even when D is empty; revision-side selection is mandatory. |
| 2026-09-05T01:07:18Z | 1 | REJECTED | Already resolved: the shell executes prepare, direct descriptor inventory builds, then refreshed reconciliation and one final lowering before execution. |
| 2026-09-05T01:07:18Z | 2 | REJECTED | Already resolved: refreshed absence can retire a previously known kernel variant, while newly discovered keys become owed work and cost contraction cannot erase valid deferred keys. |
| 2026-09-05T01:07:18Z | 3 | REJECTED | Already resolved: inner never returns successful verification exit zero; SHADOW/STALE remain distinct nonzero outcomes and otherwise inner exits 6. |
| 2026-09-05T01:33:48Z | 1 | REJECTED | Already resolved: merge rejects budgets before inventory and uses the direct repaired build-only seam. The historical claim that the old flag already stopped before QEMU was incorrect; the later harness correction is present. |
| 2026-09-05T01:33:48Z | 2 | REJECTED | Already resolved: retirement is reconciled across the whole provisional work set before lowering, with explicit reasons/counts; remaining obligations require passing outcomes. |
| 2026-09-05T01:33:48Z | 3 | REJECTED | Already resolved: merge rechecks effective content and pinned HEAD after inventory and after the final executable step. |
| 2026-09-05T03:15:07Z | 1 | REJECTED | Already resolved: build-only exits after successful compilation/staging and before QEMU. Its cold descriptor evidence and negative fixture are retained. |
| 2026-09-05T03:15:07Z | 2 | REJECTED | Already resolved: matching final content over another single parent is accepted after additional obligations are discharged; changed content is refused. |
| 2026-09-05T03:15:07Z | 3 | REJECTED | Already resolved: inner captures its effective tree before model loading/planning and checks it again before sealing usable evidence. |
| 2026-09-05T03:34:00Z | 1 | REJECTED | Already resolved: `MergeState` pins revision and parent, and both merge boundaries reject HEAD movement even when replacement commits share the same tree. |

The clean plan re-audits at 2026-09-05T09:51:10Z and 2026-09-05T10:43:26Z contain no findings to accept or reject. Their ratings remain statements about their reviewed plans.

The later responses' supplemental requirements and corrections were checked too: complete source-derived test declarations, production-manifest dependency negatives, consumed/scoped reducer payloads, inner-owned target refresh, complete profile inventory, private loader acquisition, precise manifest halt predicates, private performance images, loader timestamp isolation, reserve/subscription corrections and preserved IOMMU observation windows remain implemented. The retained performance acceptance and native trust evidence were independently checked during the preceding re-audit in this session, including 850 retained/pinned references, all eight service workflows and all 32 signed serial captures. No new defect in those resolutions was found. The cost-warning and reduced calibration dispositions remain justified by the current widening-only planner and aggregate native qualification rules. The retained measured times continue to identify their original pinned revisions; this correction claims no new benchmark timing.

**Verification and completion.** After the production and regression changes, `./check.sh --gate guest-verdict,verify-scheduler` returned 0. All 14 guest-verdict tests passed in 3.810 seconds, including both new regressions, the existing loader/media isolation checks and negative verdict histories. The scheduler gate passed its default-two-slot, explicit override, budget and failure-propagation cases. Bash syntax, Python parsing, `shfmt -d` and whitespace checks passed. One earlier focused Python invocation terminated with SIGILL before unittest output; its unchanged replay passed, and the final combined gate passed. No cause is inferred from that startup failure.
