...
AUDITOR'S RE-AUDIT ON P02M0167 (2026-09-08T11:09:44Z):

Current implementation rating: 7/10

1. **Cleanup can delete a cached fixture generation after a run selects it but before that run copies it.** The content-keyed system-disk path prevents one generation replacing another, but `media_sweep` deletes other generations older than 720 minutes whenever `fuser` sees no open descriptor (`src/harness/qemu-run.sh:252-261`). Reusing an existing template does not refresh or retain it: `qemu_prepare_system_disk` returns its pathname, and `qemu_run_disk` opens it only later (`:137-175,193-199`). Reproduced using those unchanged production helpers: prepare generation A, age its cached template two days, let run A select it again, then let run B prepare generation B before A takes its private copy. B's sweep deletes A; A's copy fails with `cannot stat`. The timestamp measures template creation, not the lifetime of the current reader, so an ordinary old cache entry defeats the claimed protection. The shared sweep also serves the other fixture formats. M3's immutable-during-the-run acquisition guarantee remains incomplete; the new overlap test uses freshly created templates and misses this cleanup interval.
...
## IMPLEMENTER'S RESPONSE TO RE-AUDIT ON P02M0167 (2026-09-08T16:31:53Z):

All 57 numbered findings in the 25 audit/re-audit sections were checked against the current implementation and M0-M5. REJECTED below means the specific finding no longer warrants a change; it does not claim that the earlier defect was imaginary. Historical execution counts, completion prose and prior responses were not treated as fresh verification.

**2026-09-08T13:58:35Z, finding 1: ACCEPTED.** The production `test-kernel.sh` and `lib.sh` publish absolute paths; the `../` prefix made all seven guest/full/scoped, host/full/scoped, development/full/scoped and build evidence arguments unusable from `src`. `verify.sh` now passes those paths unchanged to the comparison command. The explicit `RESULT-LOGS` choice remains the source of guest evidence, including architectures whose suite output is in the run log.

**Additional M5 path finding: ACCEPTED.** A repository-relative `--candidate` argument was also opened from both the repository root and `src`, so the same production route could fail to reopen its frozen input. The shell parser now resolves relative candidate arguments against its initial working directory once. Absolute arguments are retained. Candidate content, model hashes, trust bars and activation behavior are unchanged.

**Additional M1 exact-selection finding: ACCEPTED.** The NUMA gate still asked for the `numa` tag while asserting a fixed set of profile effects. Both invocation sites now request the nine existing NUMA test IDs through `TEST_SELECTION`. This preserves the topology-report test, which calls `bind_online()` and `report()` in a test boot, plus every allocator/placement assertion. Comparing declarations proved the explicit list equals the complete current nine-test NUMA set. The normal full kernel suite retains those tests; the profile no longer inherits incidental smoke tests or future additions to the tag.

`src/tools/test-shadow-logs.py`, registered in `gate-result-logs`, executes the actual shell route in a temporary tree. Only expensive producers, planner answers and comparison file reads use host fixtures; no QEMU execution or earned production candidate trust is claimed. Two tests cover dry and executable shadow routes, both published suite-log locations, all seven evidence arguments, and relative/absolute candidate inputs. All four route cases pass in approximately 3.4 seconds. Restoring each of the seven old evidence prefixes individually in a temporary script fails with the intended missing-file error. Removing relative-candidate normalization likewise fails while reopening `candidate.toml` from `src`. No production source mutation was left behind.

**Individual dispositions of the earlier numbered findings:**
...
| 2026-08-28 20:29:15 CEST | 2 | REJECTED: `check-qemu-arch-profiles.sh` selects its named assertion IDs using `TEST_SELECTION`; the broad-tag path described is gone. |
| 2026-08-28 20:29:15 CEST | 3 | REJECTED: `catalog.rs` and `commands::steps` give architecture/NUMA profiles independent keys, commands, costs and guest counts; umbrellas are not scheduled twice. |
| 2026-08-28 20:29:15 CEST | 4 | REJECTED: the acquisition lifetime gap was already corrected by PID leases and shared sweep locking. `test-kernel.sh`, `qemu-run.sh`, and `result-logs.sh` already provide staged kernel/loader inputs, per-run writable files and explicit result paths. |
| 2026-08-28 20:29:15 CEST | 5 | REJECTED: `commands::validate` rejects malformed graphs; `verify.sh` records failed/blocked IDs and waits for prerequisite verdicts before admitting dependents. |
| 2026-08-28 20:29:15 CEST | 6 | REJECTED: the earlier ordering defect is already corrected by `order_by_cost` choosing the cheapest ready step. `commands::steps` already separates build parts, gates and conformance; `History::record_step_id` and `step_seconds` already retain valid whole-step measurements. |
| 2026-08-28 20:29:15 CEST | 7 | REJECTED: candidate loading and shadow execution exist; `five_real_changes_through_the_real_planner_reach_the_threshold` edits actual temporary source bytes and exercises memory/ELF candidates and the production activation evaluator. The test-suite risk row is gone. |
| 2026-08-28 20:29:15 CEST | 8 | REJECTED: activation calls `candidate::evidence_failures`, verifies all overwritten base files, validates before materialisation, rereads the canonical model and rolls back on mismatch. No additional activation abstraction is needed. |
| 2026-08-29T16:05:00Z | 1 | REJECTED: the real boot fixture and gated staged reach select genuine block/network/USB/manager effects; missing fixtures have explicit exceptions rather than false coverage claims. |
| 2026-08-29T16:05:00Z | 2 | REJECTED: current PID leases preserve selected generations throughout acquisition; the listed kernel, fixture-name, console and socket isolation changes already exist. |
| 2026-08-29T16:05:00Z | 3 | REJECTED: `commands::steps` emits each NUMA profile separately with its own guest reservation. |
| 2026-08-29T18:29:58Z | 1 | REJECTED: truthful boot-chain coverage and explicit absent-fixture exceptions replace the obsolete unselectable-oracle claims. |
| 2026-08-29T18:29:58Z | 2 | REJECTED: PID leases and the common acquisition/sweep lock already close the template race; immutable staging and per-run captures were already implemented. |
| 2026-08-29T18:29:58Z | 3 | REJECTED: NUMA profiles are independent catalogue steps, not an indivisible umbrella. |
| 2026-08-29T23:02:31Z | 1 | REJECTED: current `Model` boot-fixture reach and actual component assertions resolve the reported selection omission; inventing coverage for absent fixtures would be incorrect. |
| 2026-08-29T23:02:31Z | 2 | REJECTED for the stated paths: `test-kernel.sh` stages the selected kernel; FAT/ISO/UDF names carry content identity and console captures carry run identity. The distinct cleanup interval is also already protected by PID leases. |
| 2026-08-30T08:40:38Z | 1 | REJECTED: the selected kernel is staged under the producer lock and run directly; there is no second unlocked Cargo test compilation of the reported form. |
| 2026-08-30T23:31:51Z | 1 | REJECTED: the medium consumes staged kernel/loader inputs; the registered concurrency gate checks the complete two-selection/two-tag boundary. |
| 2026-08-31T01:15:33Z | 1 | REJECTED: the staged medium-input boundary is implemented and no longer merely deferred to another milestone. |
| 2026-08-31T19:28:51Z | 1 | REJECTED: `check-concurrent-selection.sh` is a registered executable proof with independent selections, tags and result paths. |
...
| 2026-09-03T03:12:09Z | 1 | REJECTED: `candidate_arg` reaches shadow target/selection/build/host/dev queries and comparisons; the real candidate/change regression replaces handmade evidence. |
| 2026-09-03T03:12:09Z | 2 | REJECTED: `evidence_components`, `risk_components`, and `evidence_failures` cover registry losses, effective successor ownership and all protected risk fields before writes. |
| 2026-09-03T03:12:09Z | 3 | REJECTED: current ready-step ordering already reconsiders newly eligible work after every prerequisite. Separate runnable units, guest seeds, shared emitted pricing and explicit zero-budget handling already exist. |
| 2026-09-03T03:12:09Z | 4 | REJECTED: history excludes failed durations and unexecuted merged members; a failure under another model cannot relabel an old successful cost. |
| 2026-09-03T03:12:09Z | 5 | REJECTED: prepared scheduler execution ends with its own verdict and does not apply unrelated worktree trust admission. |
| 2026-09-03T03:12:09Z | 6 | REJECTED: the production `changes` command emits both rename paths. |
| 2026-09-03T10:37:00Z | 1 | REJECTED: effective ownership, architecture and escalation losses are checked independently of direct covers lists in `candidate.rs`. |
...
| 2026-09-07T21:49:51Z | 2 | REJECTED: `evidence_components` maps reassigned covers through effective successor ownership; the memory/ELF candidate tests grade evidence the candidate actually produces. |
| 2026-09-07T21:49:51Z | 3 | REJECTED: `a_real_test_source_change_keeps_the_declared_ids_and_all_build_targets` exercises current exact file-local selection and retained three-target builds. |
| 2026-09-08T11:09:44Z | 1 | REJECTED: `media_sweep` already obtains a PID lease under the common acquisition/sweep lock before selecting or building a generation. The existing aged-generation regression covers the selected-but-not-open interval, live private-copy retention, missing `fuser`, and dead-reader reclamation. |
| 2026-09-08T11:09:44Z | 2 | REJECTED: `commands::order_by_cost` validates first, then repeatedly chooses the cheapest currently ready step, with stable ID ties. `emit_steps` uses the same `step_cost` as emitted pricing. Existing regressions check newly ready dependents and malformed graphs. |

The original 2026-08-28 findings 4 and 7 and 2026-09-03T03:12:09Z finding 1 are rejected for their stated obsolete mechanisms (newest-log glob, missing candidate implementation and active-model selection). The separately reproduced current producer/consumer pathname defects are accepted and repaired above; the prior responses' operational-completion claims did not overrule the actual shell behavior.

Focused validation before the final phase: shell syntax, registered `gate-result-logs`, `gate-oracles` (22 named IDs), exact NUMA declaration-set comparison and `git diff --check` passed. Builds, the full Rust model suite and live guest runs were deferred until the final phase of the whole job. Consolidated execution results are recorded below.


**2026-09-08T15:04:03Z re-audit of the plan: ACCEPTED as focused confirmation.** This additional review reports no unresolved material findings and introduces no finding IDs. Its independent production-helper/caller fixture opened all seven evidence paths, its temporary negative control failed, and shell syntax passed. Those observations agree with the repaired path handling above; its 10/10 plan rating is not treated as full verification, live guest evidence or candidate trust. The consolidated final execution results remain the separate validation record.

**Completion and final verification**

The current M0-M5 runner requirements are complete within the documented exception and trust boundaries. All 146 verification-model tests, the scheduler/result-log/oracle gates and all 17 guest-verdict tests passed. The production shadow-route fixture passed two tests/four route cases; the eight pathname negative controls failed at the intended boundary during focused validation. Each NUMA architecture passed its exact nine-test selection and existing topology, allocation, placement and matrix assertions. The first aarch64/riscv64 attempts refused stale test-volume stamps before guest execution; the full cross-architecture boot build and both reruns then passed. The concurrent-selection gate passed overlapping dma/domain selections with distinct tags, media and logs, plus aged-media acquisition/reclamation. These tests do not activate a frozen candidate or establish production candidate trust.
...
AUDITOR'S RE-AUDIT ON P02M0167 (2026-09-08T16:52:35Z):

Current implementation rating: 7/10

1. **A shadow run rejected because its source changed still contributes clean candidate evidence.** The guest comparison immediately saves its record, deriving the source and per-component change digests from the tree at comparison time (`src/tools/verify-model/src/main.rs:783-786`); host, build and development comparisons do likewise. Only after those writes does `verify.sh:637-643` compare the initial and final source/model identities. Its refusal leaves every record intact. `Log::is_clean` checks the record's model, verdict, self-check and component, but has no indication that the enclosing run was declared void (`src/tools/verify-model/src/shadow.rs:848-849`). Thus an edit arriving after guest execution can receive evidence from the previous source, including the execution sample M5 requires.

   Reproduced in an isolated tracked source snapshot with unchanged production `verify.sh`, the current real model/comparison implementation, and a frozen ELF ownership-and-covers candidate. Expensive execution used host fixtures; the full guest fixture changed `src/kernel/elf.rs` after producing its results. `--shadow-exec` exited 1 with the actual moved-tree refusal, while the candidate's `TestGuest` record remained `Consistent`, `model_self_check=true`, and `shadow_exec=true` for `kernel.elf`. Subsequent production trust evaluation counted it as **one clean comparison of five required**. No candidate was activated and no complete certificate was earned; the defect is that this explicitly invalidated run receives qualifying credit at all. Evidence: `/tmp/p02m0167-shadow-candidate-result.json`, `/tmp/p02m0167-shadow-candidate-credit.log`, and `/tmp/p02m0167-shadow-drift-51lvlujm/.build/candidate-reproduction.log`.

2. **Unmeasured long host checks are admitted with a one-second minimum instead of M4's required conservative seed.** `seed_seconds(0)` returns 1 for every non-guest step (`src/tools/verify-model/src/history.rs:516-518`), and `step_cost` uses that minimum when no current measurement exists (`src/tools/verify-model/src/main.rs:1259-1263`). For example, `targeted-cache` invokes repeated real targeted builds and deliberately changes an ABI dependency to force a rebuild (`src/tools/check-shared-cache.sh:184-227`); its existing recorded whole-step duration is 571 seconds. In an isolated snapshot without history, the real emitter assigns this same step `STEPCOST 1`, with no additional prerequisite cost. The production scheduler, driven by that emitted plan with execution replaced by marker fixtures, admitted it among 71 nominal one-second steps under `--budget 71`. No expensive gate was rerun for this reproduction. This is the unresolved conservative-admission requirement at `docs/todo/P02M0167.md:466-467`, rather than a demand that estimated budgets act as timeouts: a universal nonzero floor does not conservatively price the substantial build work the selected command performs. Evidence: `/tmp/p02m0167-cold-commands.tsv`, `/tmp/p02m0167-cold-budget-result.json`, and `/tmp/p02m0167-cold-budget.log`.

Verification: read the milestone and audit/response history and checked the current planner, candidate evaluator, scheduler, oracle and artifact-acquisition paths. All 146 verification-model tests passed, together with the scheduler, component-oracle, gate-oracle and result-log checks, both shadow-path host tests, the aged media-generation regression, and all 17 guest-verdict tests. These checks and isolated reproductions do not constitute a new live-guest sweep. No repository source code or preceding audit text was changed; only this section was appended.


---

## IMPLEMENTER'S RESPONSE TO RE-AUDIT ON P02M0167 (2026-09-08T19:01:09Z):

Both findings in the 2026-09-08T16:52:35Z re-audit are **ACCEPTED** and repaired; the final cross-review also caught and repaired the fast-measurement corner described below. All 59 numbered historical findings, the prior implementation's two additional path/selection findings, and the finding-free plan review were reassessed against current M0-M5 requirements. A rejection below means no further source change is warranted for that finding's stated mechanism; it does not deny that the earlier implementation had the reported defect. Historical execution counts and earlier completion claims are not fresh verification.

| Audit timestamp | Finding | Disposition and current evidence |
| --- | --- | --- |
| 2026-09-08T16:52:35Z | 1 | **ACCEPTED.** The shell sweep now stages every guest/host/development/build comparison in a run-private pending file. It publishes only after the enclosing source/model checks pass; `Log::publish` also rejects mismatched record identities and rechecks the current source digest. An invalidated run removes its pending evidence, leaves prior canonical records byte-identical, and adds no qualifying clean-run credit. Host/development/build log files are run-private. Direct model comparison callers retain immediate recording. The boundary revalidates identity; it is not an atomic filesystem revision lock or a production candidate activation claim. |
| 2026-09-08T16:52:35Z | 2 | **ACCEPTED.** The one-second cold non-guest floor is removed. `CostModel::scheduled_seconds` returns no cost without a successful current-model whole-step/key measurement or the existing guest seed. `emit_steps` marks these costs unknown and orders unknown ready work after priced work; `verify.sh` excludes their complete dependent closures under a budget. Wholly unpriced work reports an unknown minimum, remains runnable without a budget, and becomes admissible when the persisted measurement is available. Human plan output labels provisional key estimates and states that the scheduled total is unknown. The 571-second targeted-cache observation is supporting historical evidence, not a claimed fresh build measurement or a new hardcoded seed. |
| 2026-09-08 final cross-review | additional sub-second learning finding | **ACCEPTED.** The existing shell clock records integer seconds, so a fast successful run wrote zero and the new unpriced path could not learn from it. `run_one_step` now rounds an observed successful zero-second interval up to one second. Failed intervals remain unchanged and cannot become successful costs; no unexecuted work receives this measurement. The registered scheduler fixture executes the real fast prepared command and recorder, checks the positive persisted duration, and then admits the measured command under that budget. |
| 2026-09-08T13:58:35Z | 1 | **REJECTED as a further change.** The seven producer-published absolute evidence paths are already passed unchanged. The production-route regression still covers guest-suite output in either published log, dry and executable comparisons, and every host/development/build evidence path. |
| 2026-09-08T16:31:53Z implementer | additional relative-candidate finding | **REJECTED as a further change.** Candidate arguments are already normalized once against the initial shell working directory; the route fixture covers relative and absolute frozen-candidate paths. |
| 2026-09-08T16:31:53Z implementer | additional exact-NUMA-selection finding | **REJECTED as a further change.** Both NUMA invocation sites already use the same nine exact IDs, including topology reporting and allocator/placement assertions; profile gates retain their independent keys and reservations. |
| 2026-09-08T15:04:03Z | finding-free plan review | **REJECTED as a source-change request.** It reports no unresolved finding. Its path-boundary observations remain consistent with the current route; its plan rating does not establish production trust or replace execution. |

Earlier numbered findings, individually:
...
| 2026-09-02T03:51:29Z | 2 | REJECTED: the shell matrix covers shared prerequisites, unknown costs, failure precedence and descendants. |
| 2026-09-02T12:08:00Z | 1 | REJECTED: `guests_in_flight` sums declared reservations; the scheduler test observes the two-slot/one-slot overlap limit. |
| 2026-09-02T12:08:00Z | 2 | REJECTED: guest-boot seeds and upward rounding already resolve the stated zero-cost admission. The distinct non-guest one-second seed is accepted under the newest finding 2 below and removed, rather than defended by this historical rejection. |
| 2026-09-03T03:12:09Z | 1 | REJECTED: `candidate_arg` reaches shadow target/selection/build/host/dev queries and comparisons; the real candidate/change regression replaces handmade evidence. |
| 2026-09-03T03:12:09Z | 2 | REJECTED: `evidence_components`, `risk_components`, and `evidence_failures` cover registry losses, effective successor ownership and all protected risk fields before writes. |
| 2026-09-03T03:12:09Z | 3 | REJECTED: current ready-step ordering already reconsiders newly eligible work after every prerequisite. Separate runnable units, guest seeds, shared emitted pricing and explicit zero-budget handling already exist. |
| 2026-09-03T03:12:09Z | 4 | REJECTED: history excludes failed durations and unexecuted merged members; a failure under another model cannot relabel an old successful cost. |
...
| 2026-09-07T21:49:51Z | 3 | REJECTED: `a_real_test_source_change_keeps_the_declared_ids_and_all_build_targets` exercises current exact file-local selection and retained three-target builds. |
| 2026-09-08T11:09:44Z | 1 | REJECTED: `media_sweep` already obtains a PID lease under the common acquisition/sweep lock before selecting or building a generation. The existing aged-generation regression covers the selected-but-not-open interval, live private-copy retention, missing `fuser`, and dead-reader reclamation. |
| 2026-09-08T11:09:44Z | 2 | REJECTED: `commands::order_by_cost` validates first, then repeatedly chooses the cheapest currently ready step, with stable ID ties. `emit_steps` uses the same `CostModel::scheduled_seconds` for ordering and emitted pricing. Existing regressions check newly ready dependents and malformed graphs. |

The previous broad operational-completion wording is not repeated as proof: current source movement now prevents publication rather than merely causing a late nonzero exit. Old zero-cost and ordering findings remain closed for their stated cases, while the distinct nonconservative host seed is accepted and corrected above. Missing material driver/service fixtures retain their explicit M2 exceptions; no substitute synthetic oracle, new trust model, extra runtime feature, or unrelated catalogue redesign was added.

Focused validation during implementation: the new Rust publication test passed (0.06 s), verifying deferred clean-run credit, source/model refusal, byte-preserved previous evidence and successful stable publication. The new cold-cost test passed (0.01 s), including failure/stale-history rejection, persisted measurement admission and known-before-unknown ready ordering; the existing guest-seed regression also passed. The production shadow shell fixture passed three tests/eight route cases (about 7 s), including both source/model drift and dry/executable modes. In temporary copies only, restoring immediate comparison publication reproduced four persisted records despite a final void in both modes, and the new regression rejects that behavior. The scheduler matrix passed shared unpriced prerequisites, both blocked budget branches, independent priced work, truthful unknown minimum, ordinary unbudgeted execution and measured re-admission, together with its existing failure and slot cases. Final cross-review reproduced the actual recorder’s successful/failed zero-duration outputs before correction; the added prepared-command case now proves successful fast work acquires a positive cost and fits a later budget. Shell syntax, formatting and scoped whitespace checks passed. These are short host checks with fixture producers; no long build, QEMU run, full model suite or broad verification was run during implementation.
