...
- [ ] Back the phase-0 ramdisk `StorageService` with `driver.virtio-blk` (the service stays the same component; only its backend changes from the ramdisk to a real block device)
