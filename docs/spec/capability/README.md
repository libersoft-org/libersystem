...
| `propagation.cfg` | a derived capability crossing a channel, so authority can be checked where it was narrowed |
| `transactions-batch.cfg` | two objects and a batch of two: the length at which "a refused send returns ALL of them" is a different sentence from "it" |
| `transactions-single.cfg` | one capability per message and room in the queue for two, which is where a receive can be interrupted at every depth |
| `MEASUREMENTS.md` | what each configuration costs and covered, with the digests it was measured against |
| `counterexamples/` | the behaviour that caught each deliberate mutation, one file per mutation, regenerated by the gate |
...
Each mutation's counterexample is checked in, under `counterexamples/`, one file per mutation. They
are the evidence that a mutation fails FOR THE RIGHT REASON, which used to exist only while the gate
was running - it worked in a temporary directory and removed it on the way out, so a reader who
wanted to know which behaviour caught a mutation had to re-run the suite to see one. The gate runs
those checks with a single worker, because several race to the first violation and each run then
finds a different behaviour of the same length: the files would churn on every run and the diff would
be unreadable.
