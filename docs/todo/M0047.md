...
- [ ] **`display_service_restores_the_console_surface` takes the whole suite down.** Found
  ...
  Still open, and this is where to start: find the smallest set that reproduces it - the
  remaining untried predecessor is `a_service_reports_a_bootstrap_failure`, which has no tag
  of its own - and then establish what the reset actually is. `-d int,cpu_reset` on the QEMU
  line will say whether it is a triple fault and where.
