...
- [ ] **`display_service_restores_the_console_surface` takes the whole suite down.** Found
  ...
  Two things to establish first: why the guest exits successfully instead of faulting or
  timing out, and whether the test itself is wedging the display path or is a casualty of
  something earlier in the same run. Note that the exit is *successful*, which is why the
  runner cannot tell it from a clean shutdown and says only that the marker is missing.
