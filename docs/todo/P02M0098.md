...
- [ ] Specify one portable DMA contract rather than exposing only physical addresses.
  P02M0098 owns `DmaAddress`/IOVA, the generation-bound `DmaMapping` lifecycle and one
  kernel-owned backend interface for domain create/destroy, endpoint attach/detach,
  map/unmap, invalidation completion and bounded fault delivery. A host fake backend
  exercises every completion/failure and rollback order; P02M0153 supplies the first
  concrete `virtio-iommu` implementation of this already-tested interface.
...
- [ ] State the IOMMU-absent threat model honestly. Capability isolation cannot contain
  ...
  AMD-Vi and ARM SMMU are later backends to the P02M0098 contract; P02M0153 owns the first
  `virtio-iommu` provider and its QEMU isolation gate, while P02M0098 defines the policy and
  clean no-IOMMU behavior now.
