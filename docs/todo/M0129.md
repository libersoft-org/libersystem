...
- [ ] **virtio-fs** as an explicit development-only integration backend: run it behind
  ...
  mount; LiberFS and the ordinary package/staging pipeline remain canonical. M134 does
...
- [ ] **virtio-serial multiport**: generalize `virtio-console` from one debug console to
  ...
  multiplexing an ad-hoc protocol over console bytes. Reuse M134's specification-compliant
