# M0129 - Universal standards-based driver set

Status: PLANNED AFTER M0128; M0128 IS A HARD GATE. This milestone adds every approved
...
and boards. Every task below consumes M0128's generated registry, claims, transactional
...
- [ ] **USB Video Class (UVC)**: first add a bounded camera vocabulary for format/frame
  ...
  the driver; consumers use the M0123/M0126 codec leaves.
...
- [ ] **virtio-fs** as an explicit development-only integration backend: run it behind
  ...
  mount; LiberFS and the ordinary package/staging pipeline remain canonical. M0134 does
...
- [ ] **virtio-serial multiport**: generalize `virtio-console` from one debug console to
  ...
  multiplexing an ad-hoc protocol over console bytes. Reuse M0134's specification-compliant
...
- [ ] Implement M0129 in risk-ordered waves while keeping every landed wave independently
  ...
  earlier DMA driver still follows M0128's explicit `iommu-required` versus audited
...
- Concept: M0128 (the completed binder/lifecycle foundation every task must use), the
