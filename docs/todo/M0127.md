...
Status: DONE (2026-07-25). The image-conversion and dynamically linked `/bin`
...
- [x] Restore test ownership after the physical move and decompose the monolithic kernel
  ...
  - x86 APIC module extraction result (2026-07-25): periodic LAPIC tick-counter
    ...
    Remaining thematic work is device and userspace integration module extraction.
  - SMP module extraction result (2026-07-25): completed application-processor
    bring-up coverage now lives in `smp/tests.rs`, beside the `ONLINE` and `CPU_COUNT`
    state it verifies. The new canonical `smp` tag selects this contract; focused x86_64
    fast QEMU passes 7/7 with smoke tests and tag validation continues to find 141
    canonical descriptors. Cross-core scheduling, remote wake IPIs and preemption remain
    in scheduler or root integration coverage because they cross the SMP ownership
    boundary. Remaining thematic work is device and userspace integration module
    extraction.
  - Handle, Process and x86 IDT module extraction result (2026-07-25): deterministic
    rights-subset lookup, attenuation and no-ambient-authority properties now live in
    `object/handle/tests.rs`; all HandleTable contracts carry the new canonical `handle`
    tag. The bounded dynamic-symbol registry contract now lives in
    `object/process/tests.rs` under the existing `dynamic`, `memory` and `process` tags.
    The x86 `int3` round-trip now lives in `arch/x86_64/idt/tests.rs` under the new `idt`
    tag, while the RISC-V `ebreak` round-trip remains with its separate trap coverage.
    Focused x86_64 fast QEMU passes 29/29 for `handle,dynamic` and 8/8 for `idt`; tag
    validation continues to find 141 canonical descriptors. Object-property syscalls,
    Domain quota enforcement, syscall dispatch and userspace fault scenarios remain root
    integration coverage because they cross their direct owners. Remaining thematic work
    is device and userspace integration module extraction.
  - RISC-V trap and interrupt module extraction result (2026-07-25): the resumable
    `ebreak` round-trip now lives in `arch/riscv64/traps/tests.rs`, beside the SEPC
    advance logic it verifies. IMSIC EID acquisition, binding, dispatch, unbinding and
    `lsirq` inventory coverage now lives in `arch/riscv64/interrupts/tests.rs`, beside
    the MSI registry API that owns it; both contracts also use the shared `interrupt`
    tag. A fresh RISC-V SDK/userspace preflight, focused build-only validation, focused
    `interrupt` QEMU 9/9 and complete `arch-riscv64` QEMU 11/11 pass with four harts.
    Tag validation continues to find 141 canonical descriptors. Device driver lifecycle,
    syscall and userspace interrupt scenarios remain root integration coverage because
    they cross hardware and process ownership. Remaining thematic work is device and
    userspace integration module extraction.
  - AArch64 interrupt module extraction result (2026-07-25): GICv2m SPI acquisition,
    binding, dispatch, unbinding and `lsirq` inventory coverage now lives in
    `arch/aarch64/interrupts/tests.rs`, beside the MSI registry API that owns it; both
    contracts use the shared `interrupt` tag. A fresh AArch64 SDK/userspace preflight,
    focused build-only validation, focused `interrupt` QEMU 9/9 and complete
    `arch-aarch64` QEMU 10/10 pass with four cores. Tag validation continues to find 141
    canonical descriptors. Legacy x86 INTx, syscall and userspace driver scenarios remain
    root integration coverage because they cross architecture, handles or process
    ownership. Remaining thematic work is device and userspace integration module
    extraction.
  - Domain and syscall module extraction result (2026-07-25): direct hierarchical
    charge propagation, aggregate-limit rollback, high-water accounting and refund now
    live in `object/domain/tests.rs` under the new canonical `domain` tag. The matching
    `SYS_MEMORY_OBJECT_CREATE` parent-limit enforcement and DomainStats projection remain
    root integration coverage. Stateless debug/unknown/clock dispatch and ABI revision
    handshake now live in `syscall/tests.rs`, beside the dispatch owner. Focused x86_64
    `domain,syscall` QEMU passes 16/16 with smoke tests; tag validation finds 142
    canonical descriptors and the runtime harness registers 135 architecture-enabled
    cases. Object, mapping, random, fuzz and userspace syscall scenarios remain root
    integration coverage because they cross their direct owners. Remaining thematic work
    is device and userspace integration module extraction.
  - Object property and quota module extraction result (2026-07-25): generic object
    naming through `SYS_OBJECT_PROPERTY_SET` now lives in `object/tests.rs`; Domain
    property limits and create-boundary memory/handle quota enforcement now live in
    `object/domain/tests.rs`; and DMA create-boundary quota enforcement now lives in
    `object/dma_buffer/tests.rs`. The former combined Event/Timer syscall descriptor is
    two owner tests in `object/event/tests.rs` and `object/timer/tests.rs`; random syscall
    coverage now lives with the other direct dispatch contracts in `syscall/tests.rs`.
    Focused x86_64 `object,domain,dma,syscall` QEMU passes 32/32 with smoke tests; tag
    validation finds 143 canonical descriptors and the runtime harness registers 136
    architecture-enabled cases. Mapping, IPC quota, wait, device and userspace syscall
    scenarios remain root integration coverage because they cross their direct owners.
    Remaining thematic work is device and userspace integration module extraction.
  - Directory-module layout result (2026-07-25): every non-root kernel module with a
    companion `tests.rs` now uses `<module>/mod.rs` beside it, rather than a parent
    `<module>.rs` plus a tests-only directory. The mechanical move covers 21 production
    modules across AArch64, RISC-V and x86 architecture backends, frame/heap memory,
    capability objects, scheduler, SMP and syscall dispatch; public Rust module paths are
    unchanged. Crate root, `arch/mod.rs` and `object/mod.rs` remain their existing
    directory-module roots. Full source hygiene now enforces this layout, requiring each
    non-root `tests.rs` directory to contain `mod.rs` and no legacy sibling module file.
    X86 focused kernel QEMU passes 36/36; AArch64 and RISC-V focused architecture test
    images compile after fresh preflight; tag validation continues to find 143 canonical
    descriptors. Remaining thematic work is device and userspace integration module
    extraction.
  - Channel IPC ownership result (2026-07-25): blocking `SYS_WAIT` readiness,
    `SYS_WAIT_ANY` selection, direct request/reply delivery and queue-byte charge/refund
    contracts now live in `object/channel/tests.rs` beside the Channel implementation.
    The 1 MB zero-copy capability-transfer scenario remains a root integration test because
    it crosses Channel delivery, MemoryObject mapping and the syscall ABI. Focused x86_64
    fast QEMU passes 20/20 for `channel,ipc,domain`; broader kernel QEMU passes 36/36,
    and tag validation finds 144 canonical descriptors.
  - Thematic root-harness result (2026-07-25): the root `tests.rs` now contains only
    the custom registry and shared scenario support. Its 89 integration descriptors live
    in six test-only thematic modules: `test_suites/kernel.rs`, `hardware.rs`,
    `dynamic.rs`, `boot.rs`, `services.rs` and `applications.rs`. The tag checker scans
    every suite alongside owner-local `tests.rs` files, preserving one canonical registry
    with 144 descriptors. Complete x86 test-image compilation plus AArch64 and RISC-V
    architecture-image compilation pass. Focused x86 QEMU passes 36/36 for `kernel`,
    35/35 for `drivers,dynamic,boot`, 13/13 for `audio-service,process-service`, 9/9
    for `boot` and 8/8 for `imgview`. The broader image selection still reaches its
    pre-existing lossy WebP RGB MSE assertion, and the scoped PermissionManager scenario
    fails identically on the clean pre-split revision.
    Correction (2026-07-29): that lossy WebP assertion is referred to three times in this
    document as a pre-existing unrelated failure, which was true only in the sense that this
    work did not cause it. It was not a codec defect and not environmental: the assertion
    bounded RGB error on a 2x2 image, where 4:2:0 carries one chroma sample for all four pixels,
    and those four average to neutral grey. It had never passed. The encoder matches libwebp to
    a rounding unit and was not changed; the assertion now bounds luma. See M0126 for the
    reasoning, including the size sweep that points at the wrong conclusion.
- [x] Define the factory `vol://system` hierarchy so its root contains only `hello.txt`,
  ...
  - Factory-hierarchy result (2026-07-25): the typed manifest now declares all 48 command
    ELF destinations below `bin/`, 20 internal service/helper/probe ELFs below `libexec/`
    (with ConfigService at `libexec/config_service/config_service.lsexe`), six drivers
    below `drivers/`, every library category below `lib/`, and the SDK payload at
    `components/liber_component/app.wasm`. Its declared factory files are exactly
    `hello.txt`, `motd.txt`, `audio/test.mp3` and `wallpapers/logo.webp`; the manifest
    rejects undeclared source files, duplicate destinations, role/path drift, root
    pollution and a missing SDK payload. Kernel package assembly, the shared-image builder
    and a standalone `check-volume-package` archive audit use the same destination set and
    reject stale generated program paths. ProcessService resolves named programs through a
    generated manifest table, so tools remain in `bin/` while services and internal helpers
    load from `libexec/`; ConfigService persists beside its ELF, ComponentHost reads the
    nested payload, and the old internal `bin/` path is refused. X86 QEMU passes 9/9 for
    boot, 21/21 for dynamic loading and 12/12 for ProcessService/Config; the component
    scenario passes before an unrelated slow xHCI scenario fails. X86, AArch64 and RISC-V
    test images compile, all three volume packages match the manifest, shared-cache and
    static-injection gates pass, and the checked dynamic reports regenerate cleanly.
- [x] Make the service/artifact manifest the single source of truth for destination
  ...
  - Manifest/staging result (2026-07-25): `programs`, `libraries`, `factory_files` and
    runtime-owned paths now use validated typed destinations from one model. Role-specific
    validation requires tools in `bin/`, drivers in `drivers/`, services/helpers/probes in
    `libexec/` with the ConfigService owner directory exception, category libraries in
    `lib/`, and the SDK payload in `components/liber_component/`. The kernel builder and
    standalone archive checker derive the same complete `volume.pkg` entry set; the shared
    image builder writes every executable at its declared destination, prunes obsolete
    generated `bin/`/`libexec/` outputs and rejects stale paths. Artifact metadata,
    dynamic reports, cache validation and injection gates resolve program artifacts through
    the manifest instead of assuming `bin/<name>`. X86, AArch64 and RISC-V package audits
    all report exact manifest entry sets.
- [x] Migrate every runtime consumer atomically: ProcessService and ServiceManager load
  ...
  - Runtime path result (2026-07-25): the services build script generates one lookup table
    for manifest program, factory and runtime paths. ProcessService resolves every named
    volume program from that table; DeviceManager resolves drivers from the same program
    map; ComponentHost resolves its payload, input and output paths from factory/runtime
    records; ConfigService, LogService, shell completion, ConsoleService and SessionService
    resolve their owned runtime paths without source literals. The old internal
    `bin/wasi_host.lsexe`, root `app.wasm` and root `config.tree` paths are explicitly
    rejected by package/runtime tests. Focused x86 QEMU passes 9/9 boot, 12/12 for
    ProcessService/Config and 9/9 for non-slow component scenarios; the slow ComponentHost
    scenario passes before the existing xHCI slow scenario fails.
- [x] Add layout conformance gates. Host/build tests assert the exact generated volume
  ...
  - Layout-conformance result (2026-07-25): the `volume-layout` QEMU tag now combines
    the complete SystemManager-to-Shell boot chain (including actual xHCI driver launch)
    with exact package-path, fresh writable-volume and scoped-directory contracts. A
    freshly seeded LiberFS reads the declared text, MP3 and wallpaper bytes, launches
    `echo` from `bin/`, ConfigService from `libexec/`, and ComponentHost plus its SDK
    payload from `components/`; ConfigService reloads its nested tree after replacement,
    and LogService reopens the flushed `log/boot-1` journal after replacement. A
    component-directory client reads its own output but cannot open the populated
    ConfigService tree or system journal, including through a child connection. The
    service supervisor alone holds the StorageService admin endpoint and mints the
    `libexec/config_service` and `log` scopes used by those services. Existing-disk
    policy is explicit: StorageService mounts a valid existing LiberFS unchanged even
    when its LBA 0 factory header changes; QEMU development runners recreate their disk
    when `volume.pkg` is newer, so a rebuilt pre-release image receives a fresh seed.
- [x] Run the full host suite, generated-artifact checks, package/staging audits, ELF
  ...
  - Final validation result (2026-07-25): manifest host tests pass 6/6 and service host
    tests pass 18/18. `fmt-check`, generated-output checks, source hygiene, artifact
    metadata, exact manifest-volume audits, shared-cache validation, static injection
    gates and checked dynamic reports pass. The x86_64, AArch64 and RISC-V shared-image
    and userspace builds each produce the expected 60 providers and 67 dynamic consumers;
    every architecture's `volume.pkg` exactly matches the typed manifest. The focused
    `volume-layout` QEMU selection passes 12/12 on all three architectures. ARM and
    RISC-V test boot topology now attaches a virtio GPU because their machines have no
    default VGA device while the full boot-chain contract includes DisplayService,
    ConsoleService and Shell. `PACKAGE_FORMAT.md` records the before/after source and
    system-volume trees, factory root files, private runtime directories and existing-disk
    policy. The broad `app-libs-test` also stops in a clean baseline checkout at the same
    `imgconv` indexed PNG golden assertion; the `imgconv` source is outside this layout
    change and its isolated PNG dependency tests pass.
...
- Concept: M0061/M0087 (volume-loaded programs and manifest ownership), M0123/M0125
  (prefix-free shared/executable identities), M0126 (the codec growth that exposed the
