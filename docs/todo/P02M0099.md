...
THE GATE IS THREE SLICES, NOT THIRTY-ONE ITEMS. P02M0098 was one milestone carrying the whole of the
...
- [ ] Land the three groups in order, and each group in the order it is written. The first group is
  what this system runs on today; the second is what a real machine needs before anything can be
  installed on it; the third is breadth on a USB stack that already exists. Within a group the order
  is by value rather than by cost, except that a cheap item unblocking an expensive one earns its
  place ahead of it. A later item never blocks shipping or testing an earlier finished one. On a
...
- Done when: every item in the three groups above has the bounded contract and implementation scope
  ...
  hostile descriptors, hostile completions and removal without escaping its Domain; each releases all
  resources on crash; and each passes the available QEMU/fixture plus tri-architecture build gates.
