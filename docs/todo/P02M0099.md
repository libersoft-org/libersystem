...
first three: a claim it can hold exclusively (P02M0098), a protocol to be started and stopped
through (P02M0161), and a bind that rolls back when it fails (P02M0162).
...
AND THIS IS A ROADMAP, NOT ONE PIECE OF WORK. Forty-one items across three groups, several of them
...
NARROW extension it needs and says so in its own bullet. Plain-PCI resources arrive with the first
plain-PCI driver, a firmware inventory with the first firmware-described one, a sub-function binding
unit with the first driver that is actually split. What is refused is a universal discovery framework
...
- [ ] **virtio-iommu**: P02M0098 owns the generic IOMMU/domain-mapping contract;
  [P02M0153](P02M0153.md) implements the backend and closes the first x86_64 EDU +
  virtio-net security slice. Later rollout to the remaining drivers and architectures
...
- [ ] **PCIe hotplug, AER and PME** shared bus infrastructure: process standardized slot
  ...
  event, the transition through `Stopping`, a terminal `Removed`, and a NEW generation
  after a replug so a message stamped with the old one is refused by arithmetic. A USB or
  SD child disappearing is NOT that: it withdraws a provider and its parent controller
  stays `Online`.
...
These sit on the xHCI stack that already exists, so each is a class driver rather than new controller
machinery, which is why the whole group follows the controllers. Ordered by what a person sitting at
the machine notices, with the mutating and niche transports last.
...
- [ ] Each item states three things and they fit on one line each: what it REQUIRES that already
  ...
  target without an enforcing IOMMU every DMA driver in every group still follows P02M0098's explicit
...
- [ ] **A block write maps a transferred object and copies `count * 512` bytes out of it without
  ...
  is available today. RE-VERIFIED 2026-08-28 AND NARROWED: the original finding said object type and
  rights were unchecked on the same path, and that is no longer true - `sys_memory_map` goes through
  `current_typed::<MemoryObject>(handle, ObjectType::MemoryObject, Rights::MAP)`, so a handle of the
  wrong type or without `MAP` is already refused. What is still missing is the SIZE, which is the
  whole of the copy defect, and `READ` on an object the driver reads out of.
...
every item done and TODO.md ticked, and this milestone names it in its hard gate - so the gate was
closed in fact and open on paper. Corrected.
