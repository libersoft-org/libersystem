...
The third full source pass (2026-07-04, after M0100 landed) re-verified the
M0099/M0100 machinery holds and found only low/cosmetic leftovers: two record
forms still misparsed or misserved instead of refused, one unrecorded matching
decision, and one integrity-check asymmetry.

- [x] (B1, low) The allocation type `extended_ad` (2) is not distinguished - the descriptor scan keeps the short_ad step of 8 over 20-byte records and parses garbage extents (bounded by the M0099 gates, but a misread where the refuse-not-misread rule demands Invalid). Refuse allocation forms other than short_ad, long_ad, and embedded.
...
- [x] (B4, cosmetic) The M0100 tag-location check covers the File Set and File Entries but not the Anchor or the VDS descriptors - a stale or copied anchor/descriptor passes. Verify the location there too.
...
- Concept: M0096-B4/M0097-B2 (the refuse-not-misread rule on the last two forms), M0100-B4 (the tag-location rule completed), the uniform Volume API contract.
