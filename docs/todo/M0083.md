...
The seventh full source pass (2026-07-03, after M0082 landed) found the crate
...
- Concept: M0079-B1/M0080-B1 (the disk's content must never kill the service - now applied to the last boot-time reader), the M0078 spec (whose UTF-8 rule B2 enforces at the API), and the audit track's parser rule: one format, ONE parser - the sizing loop must not re-implement what `Package::parse` already validates.
