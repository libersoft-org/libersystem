# M0126 - Image conversion tool (`imgconv`)
...
different mounted volumes. The implementation preserves the atomized M0123 model: one
...
- [x] Complete decode + encode leaf matrix. Existing `bmp` and `png` gain encoders;
  implement the already accepted M0123 leaves for PPM, QOI, TGA, PCX, ICO, APNG,
  ...
  - WebP: static and animated lossless plus lossy VP8 decode/encode. AVIF, JPEG XL
    and HEIC retain the M0123 rejection/defer decisions and are not hidden aliases.
...
- [x] Audit every supported image format against its current authoritative
  ...
  - Partial result (2026-07-17): `docs/IMAGE_FORMATS.md` now records the current
    ...
    classified gaps for every M0126 format, including PNG Third Edition (2025) with
...
- [x] Conformance, hostile-input and option tests: lossless format round-trips compare
  ...
  Correction (2026-07-29): one of these bounded lossy comparisons was itself impossible and had
  never passed. The governed 2x2 lossy WebP case bounded RGB error on an image whose four pixels
  are red, green, blue and white. Lossy WebP is 4:2:0, so a 2x2 image carries exactly one chroma
  sample - the average of all four pixels - and those four average to 127.5 in every channel,
  which is neutral grey. The colour cannot survive that size, so the bound asked for the
  arithmetically impossible and the suite was red from the day it was written.
  The encoder is not at fault and was not changed. Encoding the same input with libwebp produces
  the same greys, 76/150/29 against this encoder's 77/150/29, so the two agree to a rounding
  unit. The assertion now bounds luma, which is full resolution at any size and is what 4:2:0
  actually promises to carry.
  The reasoning that nearly went wrong is worth keeping. Sweeping sizes 2, 4, 8, 16 and 32 shows
  greyscale only at 2, which reads as a defect specific to small images; the opposite is true,
  since only at 2 do four distinct colours share a single chroma sample. A size sweep cannot
  settle an encoder question. An independent encoder can, and did.
...
- Deliberate non-blocking deferrals after the closure work above: timed animation
  ...
  Unsupported or explicitly documented; do not expand M0126 into those features merely
  to avoid moving to the next milestone.
- [x] Revisit the 96 MiB memory budget and the measurement behind it. Found 2026-07-30 while
  M0134 was proving per-launch Domains: `imgconv` cannot convert the installed 174 kB wallpaper
  inside its budget, and the pre-change build fails at it identically, so this is not fallout
  from that work - it is a limit that was always too tight for an input the system ships. The
  number came from a 4K whole-Domain peak of 84,475,904 bytes plus headroom
  (`permission_manager.rs`, `IMGCONV_MEMORY_LIMIT`), which measured one image and not the range
  of them; a budget sized from a single sample refuses whatever is bigger than that sample for
  reasons nobody can predict from the file. Measure the peak across the shipped corpus rather
  than one 4K frame, and state what the limit is sized for.
  Note that the Result below describes the budget as a "reusable aggregate Domain budget". That
  is no longer what it is: M0134 gave each launch its own Domain, so the limit is now per run,
  which is what the single-run measurement had assumed all along - a second concurrent
  conversion used to eat into the same budget.
  Result (2026-07-31): the budget is 128 MiB, set from a measurement of the thing it has to
  hold. What was wrong was never the arithmetic - it was that all three measured cases upscale
  from a 2x2 BMP or a 23x15 WebP, so the decoded INPUT costs nothing and the peak is
  essentially one output buffer. The system ships exactly one image, `wallpapers/logo.webp`,
  and it is a 3840x2160 lossy WebP: converting it holds a decoded 4K input and a 4K output at
  once, about 33 MB each, which is the term a synthetic upscale can never show.
  Measured, all four now recorded by `imgconv_governed_working_set_is_measured`:
    1920x1080 upscale        22,368,256
    3840x2160 upscale        85,135,360
    23x15 animation           2,105,344
    shipped 4K wallpaper    109,830,144
  The real conversion costs 29% more than the 4K upscale the old limit was sized from, and
  104.75 MiB against a 96 MiB quota is why it was refused. 128 MiB keeps about the same
  relative headroom the original had over its own measurement (22% against 19%), so the
  author's intent is preserved and only the measurement is replaced.
  The test measures in a deliberately generous 512 MB Domain and then compares against the
  production quota, rather than running in the production Domain. That is the difference
  between a failure that says `out of memory` and one that says `measured 109,830,144 bytes
  against a 100,663,296 byte limit`: the first is the least useful possible answer to what the
  quota should be, and it is exactly what the original setup produced.
  Two things about the measuring are worth passing on, because they cost two runs. A 4K PNG of
  this wallpaper is 2.2 MB, so media capacity was never the constraint - `cannot write output`
  was read as a small medium and the capacity was raised on that guess instead of on a
  measurement. Worse, the raise asked for 65,536 FAT16 clusters, which is past the 65,524 the
  format allows, so it produced an image the driver refuses and the identical message. That
  message does not distinguish a full medium, a malformed medium and a failed write, which is
  a defect of its own and the reason both wrong turns looked like progress.
  The production constant and the test's copy of it now name each other: a kernel test cannot
  read a userspace service's constant, and a copy that drifts asserts against a quota nobody
  enforces.
- [ ] **`imgconv_cross_volume_and_failed_overwrite_preserve_destination` hangs on x86_64.**
  The test pumps 100_000 times waiting for the harness process to terminate and it never does,
  so it fails on `assert!(process.is_terminated(), "imgconv harness exits")`
  (`test_suites/applications.rs:41`). Found 2026-08-01. It is architecture-specific: the SAME
  test passes on aarch64 (runs at 2026-07-31 15:29 and 21:36 both record `[ok]`), and it fails
  on x86_64 at `2ac7091`, `861e064` and HEAD alike, so it is inherited and not fallout from the
  multi-capability work that was in flight when it surfaced.
  How it stayed hidden is worth as much as the defect: no retained x86_64 log contains this
  test AT ALL. The recent full x86_64 runs died earlier (a dispatch regression, since fixed),
  and before that the suite was mostly exercised by tag, so the one architecture where it
  breaks is the one that never got far enough to run it. A green aarch64 suite was reported as
  "the suite passes" more than once.
  Not yet investigated, and the honest first question is not about imgconv: a process that
  never reports termination points at the process/exit path rather than at image conversion.
  Check whether the harness process actually exits and the status is lost, or whether it hangs.

- Done when: `imgconv` converts every implemented M0126 format through the same RGBA/frame
...
- Result (2026-07-17): M0126 is complete. Twelve atomized no_std codec/container leaves
  ...
  typed subsets.
- Concept: M0121/M0122 (pixel/surface vocabulary and the first image consumer), M0123
  (one codec per `.lslib` leaf), M0125 (canonical `imgconv.lsexe` artifact), the
  capability rule (`volumes` only), and the NOTES image-conversion-tool item.

## M0126a - Dynamically linked system executables (no static `/bin` tools)
...
hardening is complete. This remains a standing prerequisite for M0127 and every new
M0130/M0131 executable. M0123 delivered the loader, relocations, immutable-page sharing and
atomized `.lslib` providers, but deliberately converted only `dyn_probe`; M0126a converted
...
- [x] Make the image build own one coherent PIC provider/consumer graph. Today
  ...
  - Managed-service result (M0126a, 2026-07-18): the manifest now has an explicit
  ...
  - Second managed-service result (M0126a, 2026-07-18): four domain services retain their
  ...
  - Final volume-service result (M0126a, 2026-07-18): every native `bin/*.lsexe` is now
...
- [x] Prevent generic generated clients from defeating the library split. Today's
  ...
  - Network pilot result (M0126a, 2026-07-18): LSIDL generation emits 14 concrete
  ...
  - Process/resources result (M0126a, 2026-07-19): `process-client.lslib`
  ...
  - Config/device result (M0126a, 2026-07-19): `config-client.lslib`
  ...
  - Log/time/observability result (M0126a, 2026-07-19): `log-client.lslib`
    ...
    21/21.
  - Security/audio result (M0126a, 2026-07-19): `security-client.lslib`
    ...
    passes 21/21.
  - Audio-stream consumer result (M0126a, 2026-07-19): `play` now imports concrete
  ...
  - Volume-open result (M0126a, 2026-07-19): the first `volume-client.lslib`
  ...
  - Volume-mutation result (M0126a, 2026-07-19): `volume-client.lslib` grows to
  ...
  - Async volume-write result (M0126a, 2026-07-19): `volume-client.lslib` now owns a
...
- [x] Update dependency declarations so source ownership matches binary ownership. The
  ...
  subsystem.
- [x] Establish the permanent rule and build gates that require future M0130/M0131 tools
  ...
  in follow-up waves during M0127's path move, but `/bin` is the hard first gate. Pinned
  ...
  - Result (M0126a, 2026-07-23): Cargo metadata for the `tools` package must exactly match
    its sorted `dynamic tools volume` rows, so every future M0131 bin target fails image
    ...
    so no command package can introduce a static bootstrap path. M0130's own standing
    ...
    M0130 registration acceptance check, not unfinished M0126a implementation.
...
- [x] Make the manifest-driven image build safely incremental so the ordinary
  ...
  - Incremental foundation result (M0126a, 2026-07-19): the coherent Cargo cache is
  ...
  - Split-cache hardening result (M0126a, 2026-07-19): executable compilation and
  ...
  - Parsed provider-index result (M0126a, 2026-07-20): provider dependency and dynamic
  ...
  - Source-inventory result (M0126a, 2026-07-20): every build invocation now resolves the
  ...
  - Artifact-validation consolidation result (M0126a, 2026-07-21): audit cache state is now
  ...
  - Dynamic provider gate result (M0126a, 2026-07-21): the focused `dynamic` tag now
  ...
  - Relocation allowlist graph result (M0126a, 2026-07-21): `bootproto::elf` now
  ...
  - Runtime identity substitution result (M0126a, 2026-07-21): ProcessService now
  ...
  - Duplicate provider-export result (M0126a, 2026-07-21): the image linker now
  ...
  - Static executable injection result (M0126a, 2026-07-21): `static-image-check`
  ...
  - Undeclared dependency-edge result (M0126a, 2026-07-22): the shared image
    ...
    cases.
  - Duplicate dependency-edge result (M0126a, 2026-07-22): `duplicate-edge-check`
  ...
  - Malformed dynamic-metadata result (M0126a, 2026-07-22): the shared ELF parser now
  ...
  - Malformed symbol and relocation metadata result (M0126a, 2026-07-22): package staging
  ...
  - Dependency graph integrity result (M0126a, 2026-07-23): package staging now
  ...
  - Dependency graph bounds result (M0126a, 2026-07-23): ProcessService's resolver
  ...
  - Governed network-wave result (M0126a, 2026-07-23): all eight network tools now have
    ...
    representative. The remaining M0126a reporting work is the checked per-wave source/import,
    provider, size and test-command report.
  - Checked wave-report result (M0126a, 2026-07-23): `docs/DYNAMIC_EXECUTABLES.tsv`
    ...
    Remaining M0126a work is atomization/source-ownership completion and the broader
    size/sharing/startup acceptance gates, not wave inventory reproducibility.
  - Wave footprint and launch acceptance result (M0126a, 2026-07-23): the detailed report
    ...
    Remaining M0126a work is atomization/source-ownership completion and any unresolved
    whole-image acceptance items, not per-wave size/sharing/startup measurement.
  - Cross-consumer sharing result (M0126a, 2026-07-23): the runtime gate now proves sharing
  ...
  - Raw-object and whole-image acceptance result (M0126a, 2026-07-23): the linker now
    ...
    Remaining M0126a work is atomization/source-ownership completion and any literal
    hostile-input requirements not already covered, not size/sharing/startup accounting.
  - Import ownership audit result (M0126a, 2026-07-23): the checked detailed report now
  ...
  - Final concrete storage-client result (M0126a, 2026-07-23): `volume-client.lslib`
  ...
  - Generated storage-package split result (M0126a, 2026-07-23): `lsidl-gen` now emits
  ...
  - Generated input-package split result (M0126a, 2026-07-23): `input-proto.lslib`
  ...
  - Generated process/display-package split result (M0126a, 2026-07-23): dependency-
  ...
  - Generated security-package split result (M0126a, 2026-07-23):
    ...
    provider.
  - Generated session-package split result (M0126a, 2026-07-23):
    ...
    provider.
  - Generated config-package split result (M0126a, 2026-07-23):
    ...
    the compatibility provider.
  - Final generated-package split result (M0126a, 2026-07-23): all remaining LSIDL
  ...
  - Helper compatibility-provider removal result (M0126a, 2026-07-23): the final runtime
...
- [x] Hostile-input and tri-architecture gates: generate all provider/consumer graphs on
  x86_64/aarch64/riscv64; retain M0123's malformed dynamic/string/hash/symbol/relocation/
  ...
  architecture-specific forms); do not rely only on the historical M0123 probe subset.
  ...
  - Final gate result (M0126a, 2026-07-23): all 60 providers and 67 dynamic executables
...
- Done when: all 48 current `/bin` artifacts are PIE `ET_DYN` files with canonical
  ...
  M0130/M0131 command to satisfy the same invariant from its first staged build; no `/bin`
  ...
  static tools.
- Concept: M0123's completed loader/provider pilot, M0125 canonical `.lsexe` identity,
  M0126's full codec graph, the system image as the Rust ABI compatibility unit, and the
