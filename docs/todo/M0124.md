...
- [ ] Complete encoders for every format/profile that `play` currently accepts, with
  ...
  - Result (2026-08-08), THREE of the six: **WAV** (PCM 8/16/24/32 with canonical `fmt`, padded
  ...
  - **Still open: WavPack, Ogg Vorbis and MP3.** The first two are large but tractable in the same
    shape as FLAC. MP3 is a different order of work - psychoacoustics, the bit reservoir and
    quantisation decisions - and is the one place in this milestone where "no host FFmpeg
    dependency" is a substantial project rather than a constraint on one.
...
- [ ] Add governed `audioconv [options] <input> <output>`. Input is selected by the
  ...
  - `WavPack`, `Ogg Vorbis` and `MP3` destinations answer `NotImplemented` - not `Unsupported`,
    because the difference between "this cannot be written yet" and "this is not a format" is the
    difference a user can act on. The help says which.
