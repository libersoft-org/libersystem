...
- [x] Large files: lift the ~1 GB cap (an interim triple-indirect level, or folded straight into the M0054 extent map) so a single file reaches terabytes and beyond.
...
- Concept: Native filesystem (scaling the FS for real data arrays), Storage model (typed RelativePath; authorization is the capability layer, not FS permissions), M0043 / M0049 (the layout this widens).
