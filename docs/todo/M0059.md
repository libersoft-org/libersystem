...
M0048 detects and reads exFAT; large SD cards and USB drives (over the FAT32 4 GB
file cap) need to be writable too. exFAT is close to FAT - a cluster chain plus an
allocation bitmap - so the write path shares most of the M0048 FAT machinery.
...
- Concept: Native filesystem (compatible-FS backends behind the unified Volume API), M0048 (extends its read-only exFAT and reuses the FAT write machinery).
