...
The third full source pass (2026-07-04, after M0097 landed) re-verified the
M0096/M0097 machinery holds and found only low/cosmetic leftovers: one
incompleteness of the M0097 XAR fix, one record class that shadows real
content, and two recording/noise items.

- [x] (B1, low) The M0097 XAR fix is incomplete: `parse_record` advances the LBA by `rec[1]`, but the ROOT record in the volume descriptor (offset 156) carries its own XAR length in `r[1]` and `root_extent` ignores it - a root directory behind an XAR block parses the XAR as records. Apply the same saturating advance in `root_extent`.
...
- Concept: M0097-B1 (the XAR rule completed at the root), M0096-B5/M0097 (the listing-contract and refuse-or-serve-right rules on the remaining record classes).
