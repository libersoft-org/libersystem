# M0131 - Additional system utilities

Status: PLANNED AFTER M0127, WITH AN EXPLICIT M0132 INTERLEAVE. First land the public
storage/writer, launch-context and shared parser/walker foundations plus path-based tool
modes; then M0132 lands stdio streams/process groups/redirection; finally complete the
stdin-native M0131 modes (`tee` in particular) and the full M0131 gate. M0132 depends only
on that named foundation slice, not on M0131 being complete, so the milestones have no
...
- [ ] Storage and stream primitives required by the tools, added once to
  ...
  editor safe-save, audio-header finalization, `tee` and M0132 redirects, rather than each
...
- [x] Replace PermissionManager's ad-hoc `(args, cwd, stdout)` bootstrap sequence with a
  ...
  `$NAME` shell expansion, but make `PATH` available to `which` and the M0130 command bar
  without granting either a mutable SessionService capability. M0132 extends this same
  context with stdin/stdout/stderr endpoints instead of creating a second bootstrap format.
  This item BLOCKS all nine open items of M0132, which was found on 2026-08-05 by starting M0132's
...
- [ ] **`less.lsexe`**: an interactive pager for a path (and later a pipeline input)
  ...
  pager, while M0130's `licoview` is the richer standalone file viewer.
...
- [ ] **`kill.lsexe`**: signal a session-owned job/process capability, not an ambient
  ...
  taking its Process handle; M0132 generalizes the same operation from a single Process to
...
- [x] **`uptime.lsexe`** already exists: retain its zero-capability monotonic-clock
  implementation, canonical artifact and standing inventory test. M0131 adds only shared
...
- Concept: M0061/M0125 (thin shell and canonical executables), M0035i/M0035j/M0104/M0119
  (PTY, signals, SessionService environment/jobs and tagged tests), M0071 (streaming IPC),
  M0043-M0111 (writable volumes and unified filesystem contract), M0121/M0124 (PCM service and
