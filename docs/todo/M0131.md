...
- [x] Replace PermissionManager's ad-hoc `(args, cwd, stdout)` bootstrap sequence with a
  ...
  STILL TO DO - the environment snapshot. The field exists and is sent empty. The table lives in
  SessionService, which PermissionManager holds no client for; the shell does. So the snapshot
  travels through the `run` / `run-pipeline` ops, the launcher validates names, count and total
  bytes, and only then does `PATH` reach `which` without anyone holding a mutable session
  capability. Sending an empty list now means that arrives as a value rather than as a change of
  shape.
