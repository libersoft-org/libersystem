...
- Concept: Native filesystem (the supported-compatible-FS backends behind the unified Volume API), the layering principle (a `driver.fs.*` service; multiple FS backends behind one Volume API), M0048 (the FAT backend and the `BlockDevice` trait this reuses).
