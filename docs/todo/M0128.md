# M0128 - Declarative driver binding and lifecycle core

Status: PLANNED AFTER M0127; HARD PREREQUISITE FOR M0129. This milestone changes no
...
No concrete universal driver in M0129 may start until this milestone's completion gate
passes; otherwise each new driver would deepen the architecture M0128 exists to remove.
...
- [ ] State the IOMMU-absent threat model honestly. Capability isolation cannot contain
  ...
  AMD-Vi and ARM SMMU are later backends to the M0128 contract; `virtio-iommu` is M0129's
  first planned provider, but M0128 defines the policy and clean no-IOMMU behavior now.
...
- Done when: the existing virtio and xHCI set boots entirely through the
  ...
  tests pass before M0129 begins; aarch64/riscv64 cross-build the same generated registry
  and protocol and run the architecture-specific DMA/cache/IRQ checks available there.
- Concept: the DeviceManager capability/binding model, M0040/M0119 service supervision and
  resolver lifecycle, M0062/M0110 USB hotplug, M0109's data-driven artifact manifest, the
  ResourceManager accounting policy, and M0129 as the first consumer of the completed
