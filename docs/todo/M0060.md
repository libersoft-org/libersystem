...
ISO9660 - so M0058 covers CDs but not DVD/BR. A read-only UDF backend behind the
...
- Concept: Native filesystem (the supported-compatible-FS backends - ISO9660 / UDF among them - behind the unified Volume API), the layering principle (a `driver.fs.*` service; multiple FS backends behind one Volume API), M0048 / M0058 (the FAT / ISO9660 backends and the `BlockDevice` trait this reuses).
