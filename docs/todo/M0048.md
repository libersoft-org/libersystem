...
- Concept: Native filesystem (the supported-compatible-FS backends - FAT / exFAT / ISO9660 / UDF - behind the unified Volume API), the layering principle (multiple FS backends behind one Volume API; a `driver.fs.*` service), Storage model, M0043 (LiberFS and the `BlockDevice` trait this reuses).
