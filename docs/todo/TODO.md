...
- [ ] [P02M0099 - Universal standards-based driver set](P02M0099.md) - OPEN UNTIL EVERY ITEM UNDER IT IS DONE (2026-09-13). It was carried as a reference row that could never be ticked and was counted in no phase's open work, which is a completion criterion weakened rather than met: a row nobody may tick is a row nobody has to finish, and nothing about the work under it was ever decided to be optional. Every item stays open until it is implemented and its evidence has run. Each item names its specification and a REFERENCE IMPLEMENTATION WITH ITS PERMITTED USE, which for GPL and LGPL rows is observation only and never adaptation; four items turn out to need an AML interpreter rather than hardware; the only start that creates no shared contract is virtio-rng. WHERE IT STANDS (2026-09-16): 28 done, 47 open. `virtio-serial multiport` closed whole, which also ANSWERED the serial contract for all three of its claimants - the byte stream is the base and the line coding is a layer above it, so 16550, PL011 and CDC-ACM inherit rather than decide. NINETEEN OF THE FORTY-SEVEN ARE WAITING ON DECISIONS rather than on an implementer - an owned destination service or IDL (13 items), the firmware-node device identity (5), an AML interpreter (3), an I2C controller with a real fixture (2), a range-scoped port-I/O authority (1) and a display controller that can reach a bus (1) - which that file now counts at its head instead of stating forty-seven times separately. Those counts deliberately do not sum, because six items are blocked more than once. This row previously said 43 open and seventeen blocked on four causes; the file has held 47 since before that count was written, and two of the four rows named items they did not describe. AND THE TWO ACCEPTANCE CONDITIONS THAT READ AS A GATE ON EVERY DRIVER ITEM ARE STALE (2026-09-16): P02M0172 is COMPLETE since 2026-09-10 and its `none` DMA policy is in the tree, and P02M0162 is COMPLETE since 2026-09-09 with the bind-window measurement a step inside each driver item rather than a wait on somebody else. FIVE OF THOSE DRIVERS ARE NOW WRITTEN AND RUNNING ON x86_64 (2026-09-17): NVMe, AHCI, SDHCI, HDA and virtio-scsi, each with a host-tested decision core, a manifest entry, a QEMU model in the harness and an oracle that WRITES A SECTOR AND READS IT BACK rather than reporting that the driver reached Online. Neither is ticked: the bind window is unmeasured on every target and acceptance on aarch64 and riscv64 is a separate measurement on each, deliberately left until the whole job is done. Landing them also produced the plain-PCI resource profile every later plain-PCI driver inherits, the canonical block wire that replaced three hand-written copies, and a torn-completion defect fixed in NVMe and in the already-shipping xHCI driver. The other 16 are whole drivers, days of work each
