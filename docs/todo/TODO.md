...
- [ ] [P02M0180 - A sandboxed Bluetooth service above HCI](P02M0180.md) - OPEN (2026-09-24): `bluetooth-service` passes on x86_64 and asserts key/store-capability denial and stale-handle rejection in the guest, and the AArch64/RISC-V cross-builds pass; open are two stated departures awaiting the owner.
...
- [ ] [P02M0183 - A typed modem service](P02M0183.md) - OPEN (2026-09-24): `qemu-modem-service` passes on x86_64 with the provider, client and context bounds refused and given back, and the cross-builds and dynamic report pass; open is the existing `ipv6-peer` gate: on the development image its quiet row misses because the new services start ahead of NetworkService, and on the shipping image nine of its ten rows pass in each of three full runs, the failing row differing between runs for causes found in code this job did not change - the shell's first typed line arriving split, and one early IPv4 ping (an owner decision).
