...
- [ ] [P02M0103 - The graphics rendering foundation](P02M0103.md) - OPEN UNTIL EVERY ITEM UNDER IT IS DONE (2026-09-13), for the same reason as the driver set above: it was carried as a row that could never be ticked, which parks work instead of finishing it. `s`, `a-common`, `b` and `c` are the cross-phase foundations the rest is built on, and the parts after them follow in order; renamed because text, pointer, touch, the compositor and the application platform are all elsewhere
