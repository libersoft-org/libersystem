# M0130 - LiberCommander (`lico`, `licoedit`, `licoview`)

Status: IN PROGRESS AFTER M0127. The initial shared `lico` terminal core is a manifest-owned
...
M0130 cannot complete until M0131's public storage/transactional-writer foundation exists;
M0132 later extends the command bar with pipelines/redirection but is not required for
M0130's single-command completion gate. Build one keyboard-first, mouse-capable orthodox
...
- [ ] Add the orthodox bottom command bar beneath the panels, separate from the F-key
  ...
  immutable bounded environment snapshot supplied by M0131's governed launch context
  ...
  plus arguments through the same shared parser/launcher as the shell; when M0132 lands it
...
- [ ] File operations over one or many tagged entries: copy, same-volume move/rename,
  ...
  M0131 storage primitives and shared walkers; show current file plus per-file/total bytes,
...
- [ ] Launch `licoview.lsexe [path]` independently and from `lico`/file associations.
  It is strictly a non-editing text/raw/hex file viewer in M0130: stream files larger than
...
- [ ] Safe editor persistence: detect read-only grants and external file replacement,
  require an explicit reload/overwrite/save-as decision on conflict, and use M0131's
...
- Concept: M0035i/M0036 (PTY and pointer-capable terminal), M0038/M0061/M0119/M0125
  (PermissionManager, volume-loaded governed executables, session state and `.lsexe`),
  M0043-M0111 (capability-scoped writable volumes), M0126 (existing external image action),
  M0127 (owner directories plus the `bin`/`libexec`/`lib`/`log`/`test` volume layout), M0132 (shared pipeline and
